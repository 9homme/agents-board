# US012 — Frontend component test specification
...
|---|---|---|---|
| ProjectCard renders as a link to `/projects/{id}` | FCT-US012-001 | `web/components/Dashboard/ProjectCard.tsx` | wraps content in `<a>` pointing to `/projects/{id}` |
| ProjectCard keyboard: Tab-focusable, Enter/Space navigates | FCT-US012-002 | `web/components/Dashboard/ProjectCard.tsx` | role="link", keyboard activation routes correctly |
| ProjectCard middle-click / right-click not hijacked | FCT-US012-003 | `web/components/Dashboard/ProjectCard.tsx` | uses Next `<Link>` (not a plain click handler) so native browser behavior is preserved |
| ProjectCard preserves existing visual classes | FCT-US012-004 | `web/components/Dashboard/ProjectCard.tsx` | existing CSS class names still present on the inner `<article>` |
| Detail page: header renders project name and description | FCT-US012-005 | `web/pages/projects/[id].tsx` + `web/components/ProjectDetail/ProjectHeader.tsx` | `<h1>` contains project name; description text visible |
| Detail page: empty description shows "No description" | FCT-US012-006 | `web/components/ProjectDetail/ProjectHeader.tsx` | renders "No description" placeholder when `description === ""` |
| Detail page: loading skeleton shown during project fetch | FCT-US012-007 | `web/pages/projects/[id].tsx` | skeleton/loading indicator visible while MSW is pending |
| Tab switcher: two tabs, Documents active by default | FCT-US012-008 | `web/components/ProjectDetail/TabSwitcher.tsx` | two tab elements; "Documents" has `aria-selected="true"` when no `?tab=` param |
| Tab switcher: click "User Stories" activates it and updates URL | FCT-US012-009 | `web/components/ProjectDetail/TabSwitcher.tsx` | after click, "User Stories" `aria-selected="true"`; `router.replace` called with `?tab=user-stories` |
| Tab switcher: click "Documents" re-activates it | FCT-US012-010 | `web/components/ProjectDetail/TabSwitcher.tsx` | clicking "Documents" from User Stories state sets it active; URL updated to `?tab=documents` |
| Tab switcher: `?tab=user-stories` query param on mount activates User Stories | FCT-US012-011 | `web/pages/projects/[id].tsx` | rendering with `query.tab = "user-stories"` shows User Stories tab as active |
| User Stories tab: verbatim placeholder text | FCT-US012-012 | `web/components/ProjectDetail/UserStoriesTab.tsx` | renders exact string "Coming soon — user stories will appear here in a future release." |
| User Stories tab: no network calls made | FCT-US012-013 | `web/components/ProjectDetail/UserStoriesTab.tsx` | MSW receives zero requests when only UserStoriesTab is rendered |
| Detail page: 404 renders "Project not found" + "Back to dashboard" | FCT-US012-014 | `web/pages/projects/[id].tsx` | "Project not found" text visible; tab switcher hidden; "Back to dashboard" link to "/" present |
| Detail page: 500 renders "Failed to load project" + "Back to dashboard" | FCT-US012-015 | `web/pages/projects/[id].tsx` | "Failed to load project" text visible; "Back to dashboard" link present |

## Component tests

### FCT-US012-001 — ProjectCard renders as a Next.js Link to `/projects/{id}`
...
### FCT-US012-002 — ProjectCard: keyboard-focusable and activatable
- **Component / hook under test:** `web/components/Dashboard/ProjectCard.tsx`
- **Render with:** same fixture as FCT-US012-001; mock router.
- **MSW handlers:** none.
...
- **Notes:** The "Enter activates navigation" behavior is native to `<a>` elements and does not need a synthetic `keyDown` simulation — asserting the `href` is correct (FCT-US012-001) is sufficient for keyboard navigation correctness. If the team adds a `onKeyDown` handler, add a test for it.
- **Architecture cite:** §"State strategy" — "The Link supplies focusability, Enter activation … for free"; FE surface `ProjectCard.tsx`.

### FCT-US012-003 — ProjectCard: uses Next Link (not a plain onClick) so middle-click / right-click behavior is native
- **Component / hook under test:** `web/components/Dashboard/ProjectCard.tsx`
...
### FCT-US012-004 — ProjectCard: existing visual classes preserved on the inner `<article>`
- **Component / hook under test:** `web/components/Dashboard/ProjectCard.tsx`
- **Render with:** same fixture.
- **MSW handlers:** none.
- **Expect:**
...
### FCT-US012-005 — Detail page: project header renders name and description
...
### FCT-US012-006 — ProjectHeader: empty description shows "No description" placeholder
...
### FCT-US012-007 — Detail page: loading skeleton visible during project fetch
...
- **Architecture cite:** US012 AC "Loading state for the project header"; §"Empty / loading / error states".

### FCT-US012-008 — TabSwitcher: two tabs, Documents active by default (no `?tab=` param)
...
### FCT-US012-009 — TabSwitcher: clicking "User Stories" activates it and calls `router.replace` with `?tab=user-stories`
...
### FCT-US012-010 — TabSwitcher: clicking "Documents" from User Stories state re-activates it
- **Component / hook under test:** `web/components/ProjectDetail/TabSwitcher.tsx`
...
- **Architecture cite:** US012 AC "Switching back to the Documents tab".

### FCT-US012-011 — Detail page: `?tab=user-stories` in query activates User Stories tab on mount
- **Component / hook under test:** `web/pages/projects/[id].tsx`
...
- **Architecture cite:** US012 AC "Refresh preserves the active tab"; §"State strategy" — URL is source of truth for navigation state.

### FCT-US012-012 — UserStoriesTab: renders exact verbatim placeholder text
...
- **Architecture cite:** US012 AC "Switching to the User Stories tab"; §"Components" `UserStoriesTab.tsx` — "Renders the exact verbatim string".

### FCT-US012-013 — UserStoriesTab: no network requests made
- **Component / hook under test:** `web/components/ProjectDetail/UserStoriesTab.tsx`
...
- **Architecture cite:** US012 AC "no network call is made to fetch user stories (placeholder only)"; §"Components" `UserStoriesTab.tsx` — "No network calls".

### FCT-US012-014 — Detail page: 404 response renders "Project not found" + "Back to dashboard" link; tabs hidden
- **Component / hook under test:** `web/pages/projects/[id].tsx`
...
- **Architecture cite:** US012 AC "Project not found"; §"State strategy" — `useProject` discriminates `error.code === 'NOT_FOUND'`; API contract §1, 404 envelope `{"code":"NOT_FOUND","message":"Project not found"}`.

### FCT-US012-015 — Detail page: 500 response renders "Failed to load project" + "Back to dashboard" link
- **Component / hook under test:** `web/pages/projects/[id].tsx`
...
- **Architecture cite:** US012 AC "Project fetch fails (network/server error)"; API contract §1, 500 envelope `{"code":"INTERNAL_ERROR","message":"Failed to fetch project"}`.
