...
- `architecture.md` §"Components → Frontend" → row `MarkdownRenderer.tsx` (new — US003) and `DocumentPreviewer.tsx` (upgraded — US003 swaps the body).
...
- **In:**
  ...
  - Update / extend `DocumentPreviewer.test.tsx` if its existing assertions assumed plain rendering — switch them to assert against the rendered markdown DOM (e.g. find `<h1>` instead of looking for raw `#`). The test contract from US003 is the authority for new assertions.
- **Out:**
  - Mermaid diagram rendering — `us003_fe_mermaid_diagram` wires `MermaidDiagram` into the `code` override of this MarkdownRenderer.
  - Any change to `DocumentsTab.tsx` / `DocumentSidebar.tsx` / hooks / API client (US002 owns those; this story does not touch them).
...
## Definition of Done
- All matching unit tests in `US003_fe_unit_tests.md` pass — including the XSS test.
