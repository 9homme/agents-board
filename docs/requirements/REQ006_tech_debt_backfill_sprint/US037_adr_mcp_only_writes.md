# US037 — ADR — formalise MCP-only-writes as the permanent write API
...
- **Scenario: tech-debt items are closed with `won't-fix` strike-throughs pointing to this ADR**
  ...
  - And `docs/tech_debt.md` line 89 was already struck through (the REQ005/US030 `/api/v1/projects` POST → 405 finding) — verify with the related "no REST writes" finding
  - When this story is `done`
  - Then `docs/tech_debt.md` line 98 is struck through with `→ won't-fix per REQ006/US037 ADR (MCP-only-writes is permanent)`
...
- **Scenario: tester confirms no tests are required for this story**
  ...
  - Then `US037_be_unit_tests.md` is written but contains exactly ONE row asserting that "this story is documentation-only; no executable tests apply"
  - And `US037_fe_unit_tests.md` contains the same disclaimer
  - And `US037_e2e_tests.md` contains the same disclaimer
  - And **no Robot Framework `.robot` file is created** for US037
  - And the test report (Phase 3c) for US037 confirms zero tests, zero failures, zero skipped — by design
...
- **Scenario: tech-lead's plan for this story is "no implementation tasks"**
  ...
  - Then tech-lead writes a single task file (`US037_be_adr_verification.md` or similar) whose `Track: BE-meta` task description is: "Verify `architecture.md` (REQ006) ADR section meets all AC scenarios above. If any AC is missing, raise `ARCHITECTURE_GAP_FOUND` to route back to system-architect."
...
- **`won't-fix` strike-through phrasing.** Use the existing convention from `docs/tech_debt.md` line 14 / line 79: prefix with `~~`, suffix with `~~ → won't-fix per REQ006/US037 ADR`. Match the prevailing style.
...
- **Spec review:** This is a documentation-only story per the AC ("Scenario: tester confirms no tests are required"). `US037_be_unit_tests.md` correctly contains only the disclaimer; no UT-*/IT-*/E2E-* cases are in scope. FE and e2e specs carry the same disclaimer, and no `.robot` file was created for US037 — all consistent with the AC. Spec honesty confirmed: there is no user-observable runtime behavior to test; the deliverable is the ADR document text plus tech-debt strike-throughs, which are verified structurally.
- **Result review:** `US037_test_report.md` reports zero tests / zero failures / zero skipped by design — matches the AC's "test report confirms zero tests" scenario. No `t.Skip`/`[Tags] skip` (none possible — no tests). Deliverable-verification table confirms ADR presence + strike-throughs; existing e2e suite remained green.
- **Deliverable verification (substantive — checked directly against `architecture.md` §9 and `docs/tech_debt.md`):**
  ...
  - Scenario 8 (tech-debt strike-throughs): PASS — `docs/tech_debt.md:98` struck through with `→ won't-fix per REQ006/US037 ADR-001 (MCP-only-writes is permanent)`; sibling line 91 ("Suggested fold-into-REQ006 … REST surface") also struck through with the same pointer. No other "add REST writes" line remains unstruck.
