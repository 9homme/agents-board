# US031/be_user_story_tools_error_mapping_tests
...
**Implements:** REQ006/US031 AC (all scenarios — 27 verbatim test function names lifting `RegisterUserStoryTools` from 63.5%, including the passthrough error semantics, ≥95% per-file coverage modulo §4.5 exemptions, no production-code change). Architecture §3 US031 touch row + §4.3 cluster-2 mock-repo pattern + §4.5 exemption mechanism + §4.6 local verification command (US031 row).

## Goal
Backfill `user_story_tools.go` IT-* error-mapping tests so per-file statement coverage clears ≥95%, with the 27 verbatim test functions named in US031 AC. The file has **passthrough** error semantics on most paths (no `fmt.Errorf` wrap); assertion idiom is `errors.Is(returnedErr, mockErr)`. Tests-only.

## Scope
- **In:** Edit `services/agent-board/internal/handler/user_story_tools_test.go` to add 27 test functions per US031 AC. Use a hand-written `MockUserStoryRepo` per architecture §4.3 (or extend an existing one if present).
...
## Test contract
Dev makes the 27 verbatim test-function names from US031 AC pass. Coverage:
...
Tester's `US031_be_unit_tests.md` IT-* IDs map 1:1.

## Implementation notes
- **`user_story_tools.go` PASSTHROUGHS repo errors** on most paths (architecture §4.3 "assertion nuance" — second bullet). Assertion idiom: `assert.ErrorIs(t, returnedErr, mockErr)`. Do NOT assert on `"failed to <op>"` wrap-prefixes — `user_story_tools.go` does not wrap these. **Architecture §3 US031 row explicit note:** "verify via `errors.Is(returnedErr, mockErr)`".
...
- **Status-transition tests:** same pattern as US030 — populate `GetUserStoryFunc` to return an existing record with a known status; invoke with a `status` field that maps onto each valid transition + the invalid transition.
...
- **Coverage check command** (architecture §4.6, US031 row):
  ```
...
## Definition of done
- All 27 new test functions present with US031 AC's verbatim names; all green via the local verification command above.
...
### Files touched
- `services/agent-board/internal/handler/user_story_tools_test.go` (edited — added `errors` import + 27 verbatim US031 test functions + 1 happy-path coverage test)

### Tests added
- 27 verbatim test functions matching US031 AC names (UT-001 through UT-027)
...
**Verbatim test-function names (27/27 present):** all UT-001..UT-027 from `US031_be_unit_tests.md` confirmed present verbatim in `user_story_tools_test.go` (grep match count 27/27). Plus 1 supplemental `TestGetUserStoryTool_HappyPath` to cover the `get_user_story` success return (line 94).
...
**TDG conformance:** worktree-branch commit history (found via `git log --all --grep=US031`) shows the correct cycle: `red: test spec for all 27 user_story_tools error-mapping tests (US031)` → `green: all 28 user_story_tools tests passing at 95.3% coverage (US031)` → `refactor: chore: hand off US031 ... for review (US031)`. All prefixed red/green/refactor, all tagged `(US031)`, red-before-green ordering correct. OK.
...
**Robot dryrun / live e2e:** N/A — REQ006/US031 is a tests-only BE backfill with no `tests/e2e/REQ006_*` suite; task DoD explicitly substitutes the 3× `-race` runs (satisfied above) for live e2e.
