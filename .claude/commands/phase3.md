...
5. **Git repo is initialised with at least one commit on the working branch.** `git rev-parse --git-dir` succeeds AND `git log -1` returns a commit. Worktree isolation cannot work without a base ref. If not, abort with `GIT_REPO_NOT_INITIALISED` and instruct the user to run `git init && git add -A && git commit -m "chore: baseline"`.
...
6. **Collect dev reports.** Handle:
   ...
   - `MERGE_CONFLICT` → already handled in step 5.3 above.
