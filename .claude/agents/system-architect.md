...
2. **Read `docs/product_vision.md`** then **read `docs/system_architecture.md`**. The vision doc defines the control-tower direction (live tracking, permission gates, auto/manual mode, team chat, phase commands from UI, response streams) and the architectural principles that flow from it (full REST hierarchy, `projects.path` NOT NULL, MCP = agent write interface, HTTP = human read interface). The system_architecture.md is the single source of truth for the current system state: all existing HTTP endpoints, MCP tools, DB schema, and domain types. Use both instead of reading source code. If `docs/system_architecture.md` does not exist yet, read the repo's current Go module layout, existing handlers, repos, MCP tools, migrations, and config to establish the baseline. Do a **regression impact analysis**: for every new table column, migration change, or new entity, identify which existing endpoints and MCP tools are affected (broken INSERTs, shape changes, new required fields). Document findings in the architecture under "Breaking changes & migration notes".
...
7. **You never set `Approval: approved`** — only the orchestrator does that, after the human gives an explicit yes.
8. **After the orchestrator confirms human approval**, update `docs/system_architecture.md` to reflect the newly approved changes: add new endpoints, MCP tools, DB columns, domain types. This keeps the file accurate for the next REQ. If the file doesn't exist, create it from scratch covering the full current system state. Once `docs/system_architecture.md` is updated, exit the picture for this requirement unless the orchestrator routes an `ARCHITECTURE_GAP_FOUND` back to you in Phase 2 or 3.
