# US001 — api-server runs DB migrations at startup
...
## Out of scope
- A `/healthz` or readiness endpoint — explicitly NOT part of this requirement (the original US001 healthz approach is superseded by migrations-at-startup; the existing `GET /api/v1/projects` poll is sufficient once tables exist).
- Down-migrations / rollback at startup — startup applies `.up.sql` only.
- Seeding application/test data — that remains the (now data-only) `e2e-seed` step (US002).
...
- Knock-on: US002 makes `e2e-seed` data-only (the migration step moves here) and drops the api-server circular-dependency fix (now resolved by this story).
