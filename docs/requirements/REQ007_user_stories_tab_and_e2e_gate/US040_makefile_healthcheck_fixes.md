# US040 — Makefile `e2e-up` health-check fix + data-only `e2e-seed`
...
- **Scenario: api-server health-check poll succeeds on a fresh stack**
  ...
  - And it does NOT time out waiting for tables, because the api-server applied migrations at startup (US039)
  - (no change to the poll target is required by this story — US039 makes the existing poll work)
- **Scenario: `e2e-seed` is data-only, no migration step**
  ...
  - And it does NOT run a migration step (migrations are applied by the api-server at startup — US039)
...
- **Scenario: no regression to the web health-check**
  ...
  - When US040 changes are applied
...
## Out of scope
- Changing the api-server health-check poll target — it stays `GET /api/v1/projects` and now works because of US039 (migrations at startup). This story only fixes the mcp probe and makes `e2e-seed` data-only.
...
## Dependencies
- **US039** — migrations-at-startup makes the existing `GET /api/v1/projects` poll succeed on a fresh stack and lets `e2e-seed` drop its migration step.
...
- **D-001 REVISED — CONFIRMED:** the original api-server circular-dependency fix (re-pointing the poll at a DB-free endpoint) is **removed** from this story — it is now resolved by US039 (migrations at startup). The api-server poll stays on `GET /api/v1/projects`.
