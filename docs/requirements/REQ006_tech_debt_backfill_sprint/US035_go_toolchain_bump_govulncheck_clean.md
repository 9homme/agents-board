# US035 — Go toolchain bump to clear transitive `crypto/x509` govulncheck finding
...
- **Scenario: CI / quality-gate continues to pass**
  ...
  - And the `govulncheck` step (per REQ005/US027 soft-warn behaviour) either passes silently OR is no longer needed to soft-warn the `crypto/x509` finding
...
- **Scenario: closes tech-debt finding**
  ...
  - Then `docs/tech_debt.md` line 28 is struck through with `→ fixed in REQ006/US035`
...
**Spec review (US035_be_unit_tests.md — no e2e spec file; existing suite reused per architecture §1.2):**
- Every AC scenario maps to at least one UT-*/IT-* case:
  ...
  - "closes tech-debt line 28" → struck through with `→ fixed in REQ006/US035`.
...
**Result review (US035_test_report.md, commit `6fa0726`):**
