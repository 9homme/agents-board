...
|---|---|---|---|
| Sidebar lists documents in `updatedAt DESC, id DESC` order | FCT-US013-001 | `web/components/ProjectDetail/DocumentSidebar.tsx` | fixture with a timestamp tie asserts correct DOM order |
| Empty state: no documents | FCT-US013-002 | `web/components/ProjectDetail/DocumentsTab.tsx` | "No documents yet" message; no document fetch attempted |
| Auto-select first doc when `?doc=` absent | FCT-US013-003 | `web/components/ProjectDetail/DocumentsTab.tsx` | first item highlighted; `router.replace` called with first doc id |
| Click sidebar item updates `?doc=` via shallow routing | FCT-US013-004 | `web/components/ProjectDetail/DocumentSidebar.tsx` | click item B → `router.replace` called with `doc=B`; item B marked active |
| Deep-link to valid `?doc=` pre-selects that document | FCT-US013-005 | `web/pages/projects/[id].tsx` | doc fixture shows selected; correct item `aria-selected="true"` |
| Deep-link to bogus `?doc=` shows "Document not found"; sidebar usable | FCT-US013-006 | `web/components/ProjectDetail/DocumentsTab.tsx` | "Document not found" in previewer; no item active; other items clickable |
| Rapid click cancels in-flight prior fetch (AbortController) | FCT-US013-007 | `web/hooks/useDocument.ts` | abort called on prior controller before new fetch; only latest doc shown |
| Previewer: loading indicator during content fetch | FCT-US013-008 | `web/components/ProjectDetail/DocumentPreviewer.tsx` | loading indicator present while MSW is delayed |
| Previewer: shows doc title + updatedAt + content when loaded | FCT-US013-009 | `web/components/ProjectDetail/DocumentPreviewer.tsx` | heading, timestamp, content text all visible |
| Previewer: in-pane error + Retry on 5xx content fetch; sidebar stays usable | FCT-US013-010 | `web/components/ProjectDetail/DocumentPreviewer.tsx` | error message + Retry button; sidebar item still clickable |
| Previewer: Retry button re-issues content fetch | FCT-US013-011 | `web/components/ProjectDetail/DocumentPreviewer.tsx` | clicking Retry triggers a second MSW request for the same doc |
| List loading state: skeleton in sidebar | FCT-US013-012 | `web/components/ProjectDetail/DocumentsTab.tsx` | loading indicator present while list MSW is pending |
| List error: sidebar error + Retry; previewer shows neutral state | FCT-US013-013 | `web/components/ProjectDetail/DocumentsTab.tsx` | error message; Retry present; previewer not crashed |
| List Retry re-issues the list fetch | FCT-US013-014 | `web/components/ProjectDetail/DocumentsTab.tsx` | clicking Retry triggers a second MSW list request |
| Project-not-found 404 from list endpoint surfaces page-level error state | FCT-US013-015 | `web/pages/projects/[id].tsx` | 404 from list triggers "Project not found" via `useProject` hook (coordinated with US012) |

## Component tests

### FCT-US013-001 — Sidebar: documents listed in `updatedAt DESC, id DESC` order
...
- **Notes:** The ordering is enforced by the BE (UT-US013-010 / IT-US013-003). This FE test confirms the component renders in received order without client-side re-sorting.
- **Architecture cite:** API contract §2 — "Order: updatedAt desc, then id desc".

### FCT-US013-002 — DocumentsTab: empty state when project has no documents
...
- **Architecture cite:** US013 AC "Empty state — project has no documents"; §"Empty / loading / error states".

### FCT-US013-003 — DocumentsTab: auto-selects first document when `?doc=` absent
- **Component / hook under test:** `web/components/ProjectDetail/DocumentsTab.tsx`
- **Render with:**
...
- **Architecture cite:** §"State strategy" — "auto-selects the first list item by issuing a shallow `router.replace` with `doc=<first.id>`"; US013 AC "Documents tab loads the list for the project".

### FCT-US013-004 — DocumentSidebar: clicking a different item updates `?doc=` via shallow routing
...
- **Architecture cite:** §"State strategy"; US013 AC "Selecting a document loads its content".

### FCT-US013-005 — Detail page: deep-link `?doc=` pre-selects and loads that document
...
- **MSW handlers:**
  ...
  - `GET /api/v1/projects/p1/documents` → 200 two-document fixture (same as FCT-US013-003).
...
- **Architecture cite:** US013 AC "Deep-link to a specific document"; §"State strategy".

### FCT-US013-006 — DocumentsTab: deep-link to bogus `?doc=` shows "Document not found"; sidebar remains usable
- **Component / hook under test:** `web/components/ProjectDetail/DocumentsTab.tsx`
- **Render with:**
...
- **Architecture cite:** §"State strategy" — "detect that `doc` is set but is not in the list — do NOT auto-select; show 'Document not found'"; US013 AC "Deep-link to a document that doesn't exist".

### FCT-US013-007 — useDocument hook: rapid clicks cancel in-flight fetch (AbortController)
...
### FCT-US013-008 — DocumentPreviewer: loading indicator during content fetch
...
- **Architecture cite:** US013 AC "Loading state — content is being fetched"; §"Empty / loading / error states".

### FCT-US013-009 — DocumentPreviewer: renders title, `updatedAt`, and content when loaded
- **Component / hook under test:** `web/components/ProjectDetail/DocumentPreviewer.tsx`
- **Render with:**
...
- **Expect:**
  ...
  - `screen.getByText(/This project uses/i)` visible (content rendered — as plain text or `<pre>` acceptable in US013).
- **Architecture cite:** US013 §"Previewer specifics for US013"; API contract §3 response shape.

### FCT-US013-010 — DocumentPreviewer: in-pane error + Retry when content fetch fails; sidebar unaffected
- **Component / hook under test:** `web/components/ProjectDetail/DocumentPreviewer.tsx`
- **Render with:**
...
- **Architecture cite:** US013 AC "Error — content fetch fails"; §"Empty / loading / error states".

### FCT-US013-011 — DocumentPreviewer: Retry button re-issues content fetch
- **Component / hook under test:** `web/components/ProjectDetail/DocumentPreviewer.tsx`
- **Render with:** error state as in FCT-US013-010. `onRetry = jest.fn()`.
- **MSW handlers:** none.
- **User interactions (RTL):**
  1. `userEvent.click(screen.getByRole('button', { name: /Retry/i }))`
- **Expect:**
  - `onRetry` mock was called exactly once.
- **Notes:** The `onRetry` callback wires to `useDocument.refetch()` in the parent component. This test verifies the button calls the callback; the `refetch()` behavior is implicitly covered by FCT-US013-007 and the hook's own behavior.
- **Architecture cite:** US013 AC "clicking Retry re-issues the content fetch".

### FCT-US013-012 — DocumentsTab: list loading state shows skeleton in sidebar
- **Component / hook under test:** `web/components/ProjectDetail/DocumentsTab.tsx`
- **Render with:**
  - `projectId = "p1"`, `docQueryParam = null`.
- **MSW handlers:**
...
- **Architecture cite:** US013 AC "Loading state — list is being fetched"; §"Empty / loading / error states".

### FCT-US013-013 — DocumentsTab: list fetch error shows sidebar error + Retry; previewer neutral
- **Component / hook under test:** `web/components/ProjectDetail/DocumentsTab.tsx`
- **Render with:**
  - `projectId = "p1"`, `docQueryParam = null`.
- **MSW handlers:**
...
- **Architecture cite:** US013 AC "Error — list fetch fails"; §"Empty / loading / error states"; API contract §2, 500 envelope.

### FCT-US013-014 — DocumentsTab: list Retry re-issues the list fetch
- **Component / hook under test:** `web/components/ProjectDetail/DocumentsTab.tsx`
...
- **Architecture cite:** US013 AC "clicking Retry re-issues the list fetch".

### FCT-US013-015 — Detail page: 404 from list endpoint surfaces page-level missing state
- **Component / hook under test:** `web/pages/projects/[id].tsx`
