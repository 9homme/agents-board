...
## Regression bar (architecture.md §10.1, §3 US015 row)
...
| Zero `DB_URL` in any new recipe | UT-006 | `grep 'DB_URL' Makefile` returns zero matches in new recipe lines | US010 alignment |
...
- **Architecture cite:** architecture.md §3 US015 row DELETE marker; US015 AC `startup.sh` scenario

---

### UT-002 — `shutdown.sh` deleted
- **Command:** `ls shutdown.sh` → should exit non-zero
- **Architecture cite:** architecture.md §3 US015 row DELETE marker

---

### UT-003 — No remaining references to deleted scripts
- **Command:** `git grep -nE 'startup\.sh|shutdown\.sh'`
- **Expect:** zero output (excluding the US015 story file and sign-off log per US015 AC exact wording)
- **Architecture cite:** US015 AC "git grep -nE 'startup\.sh|shutdown\.sh' returns zero hits across the working tree (excluding this story file and the REQ006 sign-off log)"
...
- **Architecture cite:** US015 AC `PG_CONN ?=` scenario; D-014 Option A union; tech_debt.md line 86
...
- **Architecture cite:** architecture.md §3 US015 row; D-013 Q1/Q3
...
- **Architecture cite:** US015 AC "DATABASE_URL is the only DB env var used in any new dev-* recipe"
...
- **Architecture cite:** architecture.md §3 US015 row
...
- **Architecture cite:** US015 AC "existing make e2e-* targets are byte-identical"
...
- **Architecture cite:** architecture.md §10.1 live-e2e + 3-clean-run mandate for US015
...
- **Architecture cite:** US015 AC "PG_CONN env-override respected"
...
- **Expect:** the resolved `psql` command uses the overridden URL
- **Architecture cite:** US015 AC "DEV_PG_CONN env-override respected"
