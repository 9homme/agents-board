# US027/be_audit_repo_error_tests
...
**Implements:** REQ006/US027 AC (all scenarios — 6 verbatim test function names covering 3 error branches × 2 public callers of `getAuditTrail`, ≥95% per-file coverage, no production-code change). Architecture §3 US027 touch row + §4.2 cluster-1 sqlmock pattern + §4.6 local verification command (US027 row).

## Goal
Backfill `audit_repo.go` error-branch tests so per-file statement coverage clears ≥95%, with the 6 verbatim test functions named in the US027 AC (3 error branches × 2 callers — typically `GetTaskAuditTrail` and `GetUserStoryAuditTrail`), following architecture §4.2. Tests-only.

## Scope
- **In:** Edit `services/agent-board/internal/repo/audit_repo_test.go` to add 6 test functions per US027 AC. Use sqlmock `_QueryError` / `_ScanError` / `_RowsErr` shapes from architecture §4.2 against the shared `getAuditTrail` plumbing.
...
## Test contract
Dev makes the 6 verbatim test-function names from US027 AC pass (typically the cross-product of {`_QueryError`, `_ScanError`, `_RowsErr`} × {`GetTaskAuditTrail`, `GetUserStoryAuditTrail`}). Tester's UT-* IDs in `US027_be_unit_tests.md` map 1:1 onto these names.

## Implementation notes
- **Reference pattern:** architecture §4.1 / §4.2 — same shape as US025 / US026, but the SUT is the `ListXxx` family because `getAuditTrail` is a query-then-scan-loop method.
- **Branch idioms used:**
...
- **Coverage check command** (architecture §4.6, US027 row):
  ```
...
## Definition of done
- All 6 new test functions present with US027 AC's verbatim names; all green via `cd services/agent-board && go test ./internal/repo -cover -v -run TestAuditRepo`.
...
**TDG discipline:** commits on branch follow red→green→refactor cycles with `(US027)` traceability tags. The `refactor: chore:` prefix variant is a known pre-existing pattern already filed in `docs/tech_debt.md` (2026-06-05 entry).
