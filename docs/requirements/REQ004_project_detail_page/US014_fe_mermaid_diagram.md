...
Status: completed
Track: FE
Implements: US014 AC "Mermaid diagrams render as SVG", "Invalid mermaid source does not crash the page", "Switching documents re-renders mermaid for the new content"
Blocked by: US014_fe_markdown_renderer.md
...
- `architecture.md` §"Components → Frontend" → row `MermaidDiagram.tsx` (new — US014).
...
- **In:**
  ...
  - Document-switch test (component-level, in `DocumentPreviewer.test.tsx` or `DocumentsTab.test.tsx` — dev's call):
    - Render the previewer with document A containing a mermaid block, switch to document B containing a different mermaid block, assert that B's mock-svg is shown and A's is gone. This validates the `key={document.id}` cleanup (already wired by US013 FE; this test confirms it still holds after the markdown swap).
- **Out:**
...
## Definition of Done
- All matching unit tests in `US014_fe_unit_tests.md` pass.
...
- `web/components/ProjectDetail/MermaidDiagram.test.tsx` — new tests covering FCT-US014-008 and FCT-US014-009.
...
- `web/components/ProjectDetail/MarkdownRenderer.test.tsx` — added `jest.mock('mermaid')`, FCT-US014-008 (code-override routing), FCT-US014-010 (no mermaid import for non-mermaid docs) tests.
- `web/components/ProjectDetail/DocumentPreviewer.test.tsx` — added `jest.mock('mermaid')`, FCT-US014-011 (document-switch resets mermaid SVG) test.

### Tests added (this task)
- `MermaidDiagram.test.tsx`: 4 tests (FCT-US014-008 × 2, FCT-US014-009 × 2)
- `MarkdownRenderer.test.tsx`: 5 new tests (FCT-US014-008 code-override × 3, FCT-US014-010 × 1, plus existing 12 unchanged)
- `DocumentPreviewer.test.tsx`: 1 new test (FCT-US014-011)
...
2. **Cleanup + key-driven swap (FCT-US014-011):** `MermaidDiagram.tsx:112-114` returns a `() => { cancelled = true }` cleanup; setters gated on `if (cancelled) return`. `DocumentPreviewer.test.tsx:174-238` proves doc-A SVG is gone and doc-B SVG present after `rerender` with a new `key`. PASS.
3. **Error fallback non-propagating (FCT-US014-009):** try/catch wraps both `import('mermaid')` and `mermaid.render`; failure sets `status: 'error'`; success path emits SVG without raw source (the `<pre>` only appears in the error branch). PASS.
4. **`mermaid.render` not called for non-mermaid (FCT-US014-010):** `CodeBlock` in `MarkdownRenderer.tsx:137` only branches to `<MermaidDiagram>` when `className?.includes('language-mermaid')`. Test at `MarkdownRenderer.test.tsx:347-356` asserts `expect(mermaid.render).not.toHaveBeenCalled()` after rendering a Go fence. PASS.
