*** Settings ***
Documentation    E2E tests for US043 — User story detail + tasks in side drawer.
...                E2E-US043-001: clicking a card opens the drawer with detail + tasks.
...                E2E-US043-002: switching stories updates the drawer; Story 2 (0 tasks) asserts
...                the empty-state "No tasks for this story." as a sub-step; Escape closes the drawer.
...                (Standalone E2E-US043-003 removed — empty-state is a sub-assertion of E2E-002.)
Library          Browser
Library          String
...
E2E-US043-001 Clicking a card opens detail drawer with tasks
    ...
    [Tags]    US043    regression

    New Page    ${WEB_BASE_URL}/projects/${PROJECT_ID}
    Click    role=tab >> text=User Stories
    
    # Wait for cards to appear
    Wait For Elements State    role=heading >> text=US043 Story 1    visible    timeout=10s
    
    # Click the card
    Click    text=US043 Story 1
    ...
    Wait For Elements State    role=heading >> text=US043 Story 2    visible
