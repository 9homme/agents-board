...
- ~~2026-06-03 — services/agent-board/cmd/mcp-server/sse.go,message.go — pre-existing govulncheck finding on stdlib crypto/x509 (transitive via Go stdlib) — bump Go toolchain or pin a fix when a runtime upgrade is available — REQ005/US005/be_project_repo_error_tests~~ → fixed in REQ006/US012
...
- ~~2026-06-03 — Makefile:16 PG_CONN — hardcoded `localhost:15432` makes the host-Postgres path the runbook promises (point at local `:5432`) non-functional. Change to `?=` so env can override — REQ005/US008/makefile-host-postgres-path~~ → fixed in REQ006/US015 (absorbed from former US011 for line 86; PG_CONN now ?= and env-overridable)
...
~~**Suggested fold-into-REQ006:** "e2e suite portability + REST surface" — adds mcp-server to compose AND adds the missing REST endpoints AND rewrites suite-setups to use REST. ~1 week of work, mostly mechanical.~~ → won't-fix per REQ006/US014 ADR-001 (MCP-only-writes is permanent; mcp-server-in-compose was completed in REQ005/US008)
...
- ~~2026-06-03 — services/agent-board/cmd/mcp-server/main.go:30 vs cmd/api-server/main.go:45 — pre-existing env-var name inconsistency (`DB_URL` vs `DATABASE_URL`) between the two binaries. Both binaries should accept both names with one preferred — REQ001-004/tech-debt/env-var-harmonisation~~ → fixed in REQ006/US010 (standardised on DATABASE_URL; DB_URL rejected at startup)
- ~~2026-06-03 — services/agent-board/cmd/api-server/main.go — api-server only exposes 4 GET endpoints; ALL data writes go through MCP. Architecturally, this is by design (MCP-as-write-API). BUT it means: no FE-driven create-update-delete flows, no REST API for non-MCP clients, and e2e setups must always use MCP. If the project ever wants browser-direct CRUD, add REST POST/PUT/DELETE endpoints — REQ001-004/architecture/no-rest-writes~~ → won't-fix per REQ006/US014 ADR-001 (MCP-only-writes is permanent)
...
- 2026-06-06 — docs/tech_debt.md:50-55 — the six 2026-06-03 project_tools.go coverage-gap entries (RegisterProjectTools 0%, handlers 75-100%) are now RESOLVED by US004 (per-file ≥95%, avg 99.25%); leave a retrospective pass to prune resolved backlog lines so the file stays a live backlog rather than a historical log — REQ006/US004/be_project_tools_error_mapping_tests
- 2026-06-07 — docs/requirements/REQ006_tech_debt_backfill_sprint/US006_be_task_tools_error_mapping_tests.md:58-91 (task `## Notes`) — the dev's hand-off Notes claim "Used hand-written MockTaskRepo" but `task_tools_test.go` actually uses `testify/mock` consistently (no style mixing). Both styles are explicitly allowed for US006 per architecture §4.3, so this is not a code defect — but the Notes-vs-reality drift could mislead a future reader auditing mock conventions. Suggested fix: be-dev hand-off Notes should state the mock style actually used. Non-blocking — REQ006/US006/be_task_tools_error_mapping_tests
- 2026-06-07 — services/agent-board/internal/handler/user_story_tools_test.go (commits 0398449, 653d89e) — same `refactor: chore:` TDG prefix drift recurs on US007 (claim-task + review hand-off commits tagged `refactor:` rather than a true refactor cycle). red→green→refactor ordering and `(US007)` tag are honored. This is now the third recurrence (US001, US004, US007); the TDG skill / be-dev prompt should gain a real `chore:`/housekeeping prefix. Cross-reference the REQ006/US001 and US004 entries — REQ006/US007/be_user_story_tools_error_mapping_tests
- 2026-06-07 — services/agent-board/internal/mcp/server.go:115 ListTools — doc-comment claims "returns the names of all registered tools in lexicographic order" but the implementation iterates a map and does NOT sort (order is unspecified). OQ-4 carry-forward from REQ006/US009 — deliberately NOT silently fixed during the tests-only US009 task; UT-006 asserts unordered membership via `assert.ElementsMatch`. Suggested fix (pick one in a future story): (a) add `sort.Strings(names)` before return to make the doc-comment true, or (b) correct the doc-comment to "in unspecified order". Non-blocking — no caller currently relies on ordering — REQ006/US009/be_mcp_server_toolregistry_tests
- 2026-06-07 — docs/requirements/REQ006_tech_debt_backfill_sprint/US012_be_unit_tests.md:23 (UT-001) — spec asserts "a `toolchain go1.26.4` directive exists on the immediately following line" in go.mod, but `go mod tidy` under go1.26.4 strips a redundant `toolchain` line when `go == toolchain`, and re-adding it breaks `go build`. The committed `go 1.26.4`-only form is the correct idiomatic Go 1.21+ shape. UT-001 sub-assertion waived at review; tester should reword UT-001 to "go.mod pins toolchain to 1.26.4 via `go 1.26.4` (toolchain directive omitted when language version == toolchain version)" on next touch — REQ006/US012/be_go_toolchain_bump_1_26_4
- 2026-06-07 — Makefile:`e2e-up` target — two pre-existing health-check bugs prevent `make e2e-up && make e2e-seed` from completing on a fresh stack: (1) api-server health-check polls `GET /api/v1/projects` which requires DB tables that only exist after `make e2e-seed` runs — circular dependency causing `make e2e-up` to time out; (2) mcp-server health-check runs `curl http://localhost:8081/sse` without `--max-time`, SSE endpoint holds connection open indefinitely causing the curl to hang. Workaround accepted: `podman-compose up -d` + `make e2e-seed` + `make e2e-run` ×3 is equivalent evidence. Fix: (1) add a dedicated `/healthz` endpoint to api-server and poll that instead; (2) add `--max-time 5` to the mcp health-check curl — REQ006/US012/be_go_toolchain_bump_1_26_4

### 2026-06-08 — REQ007/US001 review-pass-2 observations (tech-lead)

- 2026-06-08 — agent/us001be commit 54307cc — TDG commit subject uses a doubled prefix `refactor: chore: set US001 in_review after TDG rework (US001)`. The subject does start with a valid TDG prefix (`refactor:`) and carries the `(US001)` tag, so it is not a TDG-conformance violation — but the doubled `refactor: chore:` is the same prefix-vocabulary drift already filed for REQ006/US001, US004, US007. Reinforces that the TDG skill / be-dev prompt needs a dedicated `chore:`/housekeeping prefix for status-flip/hand-off commits. Cross-reference the REQ006 entries — REQ007/US001/be_migrations_at_startup
- 2026-06-08 — services/agent-board/internal/migrate/migrate.go:35 (rows.Scan) and migrate.go:42-44/61-63 (fs.ReadDir / fs.ReadFile) — three `return err` sites in `migrate.Run` have NO corresponding UT case in US001_be_unit_tests.md (spec names 6 cases covering create-table, query, BeginTx, exec, insert, commit; it omits the row-scan failure and the FS read failures). These unscanned branches are why migrate.go sits at 81% rather than higher. This is a SPEC GAP (tester-owned), not a dev gap — surfaced to the orchestrator as SPEC_GAP_FOUND for tester revision; recorded here for the REQ007 retrospective in case the orchestrator defers the spec fix — REQ007/US001/be_migrations_at_startup
