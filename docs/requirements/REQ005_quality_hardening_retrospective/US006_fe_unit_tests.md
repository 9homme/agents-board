# US006 — Frontend component test specification
...
|---|---|---|---|
| `fetchProject` signature accepts optional `signal` | FCT-US006-001 | `web/lib/api/projects.ts` | compiles with and without signal; signal forwarded to fetchClient |
| `fetchProjects` signature accepts optional `signal` | FCT-US006-002 | `web/lib/api/projects.ts` | compiles with and without signal |
| `useProject` aborts on unmount | FCT-US006-003 | `web/hooks/useProject.ts` | controller.abort called on cleanup; state not updated after unmount |
| `useProject` aborts on id change (rapid navigation) | FCT-US006-004 | `web/hooks/useProject.ts` | stale A response does not commit when B is pending |
| `useProject` AbortError silently swallowed | FCT-US006-005 | `web/hooks/useProject.ts` | error state not set; isLoading not left true on abort |
| `useProject` latestIdRef prevents stale commit | FCT-US006-006 | `web/hooks/useProject.ts` | only the latest id's response commits to state |
| `useProjectDocuments` aborts on unmount | FCT-US006-007 | `web/hooks/useProjectDocuments.ts` | cleanup aborts in-flight; no state update after unmount |
| `useProjectDocuments` aborts on projectId change | FCT-US006-008 | `web/hooks/useProjectDocuments.ts` | stale response does not overwrite newer |
| `useProjectDocuments` AbortError silently swallowed | FCT-US006-009 | `web/hooks/useProjectDocuments.ts` | no error state on abort |
| `useProjectDocuments` refetch() creates new controller | FCT-US006-010 | `web/hooks/useProjectDocuments.ts` | refetch() aborts previous in-flight; new request begins |
| `useDocument` pattern unchanged (regression guard) | FCT-US006-011 | `web/hooks/useDocument.ts` | existing tests still pass; no new failures |
| Happy path: data and loading state correct on success | FCT-US006-012 | `web/hooks/useProject.ts` | data populated; isLoading false; error null |

## Component tests

### FCT-US006-001 — `fetchProject` accepts optional `signal` and forwards it
...
### FCT-US006-002 — `fetchProjects` accepts optional `signal`
...
### FCT-US006-003 — `useProject` aborts on unmount; state not updated after unmount
...
### FCT-US006-004 — `useProject` aborts stale request on rapid id change
...
### FCT-US006-005 — `useProject` AbortError silently swallowed (no error state)
...
### FCT-US006-006 — `useProject` `latestIdRef` prevents stale `.then` commit
...
- **Notes:** This case is subtly different from FCT-US006-004: here the abort may NOT fully cancel proj-A (the mock may resolve anyway after the delay), so the `latestIdRef` guard is the last line of defence.
- **Architecture cite:** architecture §4.3 — `.then` guard `if (latestIdRef.current === id)`.

### FCT-US006-007 — `useProjectDocuments` aborts on unmount
...
- **Test steps:** same shape as FCT-US006-003 but with `useProjectDocuments("proj-001")`.
...
### FCT-US006-008 — `useProjectDocuments` aborts stale request on projectId change
...
### FCT-US006-009 — `useProjectDocuments` AbortError silently swallowed
...
### FCT-US006-010 — `useProjectDocuments` `refetch()` aborts previous in-flight and creates new request
...
### FCT-US006-011 — `useDocument` pattern unchanged (regression guard)
...
- **Architecture cite:** US006 AC "Scenario: `useDocument` pattern unchanged"; architecture §4.6.

### FCT-US006-012 — `useProject` happy path: data populated, isLoading false, error null
