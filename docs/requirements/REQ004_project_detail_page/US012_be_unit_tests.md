# US012 — Backend unit & integration test specification
...
|---|---|---|---|---|
| GET project — happy path | unit | UT-US012-001 | services/agent-board / internal/handler | `handler.GetProject(c echo.Context)` |
| GET project — 404 not found | unit | UT-US012-002 | services/agent-board / internal/handler | `handler.GetProject(c echo.Context)` |
| GET project — 500 internal error | unit | UT-US012-003 | services/agent-board / internal/handler | `handler.GetProject(c echo.Context)` |
| GET project — integration round-trip | integration | IT-US012-001 | services/agent-board | `GET /api/v1/projects/{id}` |
| GET project — integration 404 | integration | IT-US012-002 | services/agent-board | `GET /api/v1/projects/{id}` |
| Route registered on Echo | integration | IT-US012-003 | services/agent-board / cmd/api-server | `main.go` router registration |

## Unit tests

### UT-US012-001 — GetProject handler: 200 happy path
...
### UT-US012-002 — GetProject handler: 404 not found
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.GetProject(c echo.Context) error`
- **Given:**
...
### UT-US012-003 — GetProject handler: 500 internal error
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.GetProject(c echo.Context) error`
- **Given:**
...
### IT-US012-001 — GET /api/v1/projects/{id} — found
- **Service:** `services/agent-board`
...
### IT-US012-002 — GET /api/v1/projects/{id} — not found
- **Service:** `services/agent-board`
...
### IT-US012-003 — Route registration smoke test
- **Service:** `services/agent-board`
...
- **Notes:** This is a low-cost guard against accidental omission of the `e.GET(...)` call. Does NOT exercise the handler logic (covered by UT-US012-001/002/003).
