...
### 2026-06-05 — REQ006/US001 review-gate observations (surfaced during tech-lead review)

- 2026-06-05 — services/agent-board/Dockerfile:31 — missing `USER` directive (semgrep `dockerfile.security.missing-user.missing-user`) — runtime would execute as root; add `USER non-root` (or numeric `USER 1000`) before the final `CMD ["./api-server"]` — pre-existing since REQ005/US008 commit 7b836c5; surfaced as cross-gate FAIL by REQ006/US001 review pass 1 — REQ005/US008 → carry into a follow-up Docker-hardening task
- 2026-06-05 — web/Dockerfile:48 — missing `USER` directive (semgrep `dockerfile.security.missing-user.missing-user`) — same as above; add `USER non-root` before `CMD ["npm", "start"]` — pre-existing since REQ005/US008 commit 7b836c5; surfaced as cross-gate FAIL by REQ006/US001 review pass 1 — REQ005/US008 → carry into a follow-up Docker-hardening task
