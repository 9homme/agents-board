# US021 — Move `@testing-library/dom` from `dependencies` to `devDependencies`

**Story:** US021 — Move `@testing-library/dom` from `dependencies` to `devDependencies` in `web/package.json`
**Requirement:** REQ005
**Track:** FE
**Status:** completed
...
`web/package.json` is a shared scaffold file in principle, but no other REQ005 task touches it (architecture §2 US024 row explicitly says `web/package.json` is NO change under the chosen ref-attach path). Therefore this task does not need to be a scaffold gate for US024. Still, the orchestrator should treat package.json as overlap-sensitive and avoid co-picking unrelated FE tasks with this one.

## Test contract

The dev must make these tests pass (from `US021_fe_unit_tests.md`, IDs assigned by tester):
...
- Per architecture §2 US021 row and R2: keep the version specifier identical, keep alphabetical sort in the destination block, regenerate lockfile via `npm install` (NOT `npm install --package-lock-only` — we want full resolution to catch peer changes).
...
- Code matches architecture §2 US021 row.
...
- TDG skill invoked at red/green/refactor phases (commits tagged `(US021)`).
...
- FCT-US021-01 through FCT-US021-06 all PASS: dep location correct, version preserved, `npm ls --omit=dev` shows it only as transitive of `@testing-library/user-event`, 107 tests / 17 suites pass, `npm run typecheck` clean, zero production-source imports of `@testing-library/dom`.
...
- **NOTE on review modality:** Tech-lead subagent review aborted with `STALE_WORKTREE_BASE` (US023 bug). Inline orchestrator review used as recovery.
...
- `cd web && bash test/us007-package-hygiene.sh`: **3/3 PASS** (FCT-US021-01, -02, -06).
