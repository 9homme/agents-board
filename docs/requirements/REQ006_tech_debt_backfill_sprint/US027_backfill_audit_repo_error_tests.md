# US027 — Backfill `audit_repo.go` error-branch tests
...
- **`task_repo.go`** — US025.
- **`user_story_repo.go`** — US026.
- **`document_repo.go` / `project_repo.go`** — REQ005/US029.
- **Audit trail write-side coverage.** The audit trail is **written** by `task_repo.UpdateTaskStatus` and `user_story_repo.UpdateUserStoryStatus` — those write paths are covered in US025 and US026 respectively. US027 covers only the read path.
