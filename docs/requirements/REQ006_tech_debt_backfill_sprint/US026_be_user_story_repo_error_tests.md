# US026/be_user_story_repo_error_tests
...
**Implements:** REQ006/US026 AC (all four scenarios — 12 verbatim test function names, ≥95% per-file coverage modulo §4.5 exemptions, no production-code change, existing suite still green). Architecture §3 US026 touch row + §4.2 cluster-1 sqlmock pattern + §4.5 exemption mechanism + §4.6 local verification command (US026 row).

## Goal
Backfill `user_story_repo.go` error-branch tests so per-file statement coverage clears ≥95% (modulo enumerated unreachable lines), with the 12 verbatim test functions named in the US026 AC, following the architecture §4.2 sqlmock pattern. Tests-only — `user_story_repo.go` is byte-for-byte unchanged.

## Scope
- **In:** Edit `services/agent-board/internal/repo/user_story_repo_test.go` to add the 12 functions enumerated in US026 AC. Use the sqlmock branch→shape mapping from architecture §4.2. For `_AuditInsertError` / `_CommitError` declare `ExpectRollback()` per the §4.2 note on `user_story_repo.go:65-71`.
...
## Test contract
Dev makes these spec IDs pass (from `US026_be_unit_tests.md` once tester authors it). The 12 verbatim function names from US026 AC are the authoritative list. Same shape as US025 (Create/Get/Update split, plus `UpdateUserStoryStatus` 5-branch transactional set, plus `ListUserStories` Query/Scan/RowsErr triplet). Tester's UT-* IDs map 1:1 onto these names.

## Implementation notes
- **Reference pattern:** architecture §4.1 — `document_repo_test.go` and `project_repo_test.go` from REQ005/US029; this story is its lineal descendant.
- **Branch → sqlmock idiom:** identical to US025 — see architecture §4.2 mapping table verbatim.
- **`UpdateUserStoryStatus` runs inside a transaction** (`BeginTx` → `QueryRowContext` → `ExecContext` → `Commit`) with five distinct error exits. Tester may consolidate via `t.Run` per US026 AC; either shape acceptable.
...
- **Coverage check command** (architecture §4.6, US026 row):
  ```
...
## Definition of done
- All 12 new test functions present with the exact verbatim names from US026 AC; all green via `cd services/agent-board && go test ./internal/repo -cover -v -run TestUserStoryRepo`.
...
- **Production code unchanged.** `git diff main..HEAD -- services/agent-board/internal/repo/user_story_repo.go` produces zero output. Confirmed byte-for-byte unchanged per task DoD + architecture §3 US026 row.
...
- **TDG conformance:** verified commit history `6e3f75a red: test spec for UT-001..UT-013 error-branch tests (US026)` → `4778878 green: user_story_repo.go error-branch tests pass (US026)` → `d4dff2c refactor: add empty-result test to reach ≥95% ListUserStories coverage (US026)` → `29ad84e refactor: chore: hand off ... for review (US026)`. Strict red → green → refactor sequence; every subject carries `(US026)` traceability tag. OK.
...
6. Flip US026 (and US025, US036) from `blocked_review_gate` back to `in_review` for tech-lead re-review.

Once the Dockerfile fix lands and the cross gate emits PASS, re-review of THIS task will be a one-pass approval — the code-review checklist above is already entirely clean. No code change is requested of be-dev for US026.

### Review pass 2 — 2026-06-05 — verdict: approved

**Context:** Pass 1 returned `blocked_review_gate` because the cross-gate semgrep `dockerfile.security.missing-user.missing-user` finding (pre-existing in `services/agent-board/Dockerfile:31` + `web/Dockerfile:48`) blocked PASS, even though the US026 code under review was unimpeachable. The orchestrator routed a gate-fix task; commit `a22562e fix(docker): add non-root USER to api-server and web images` added `USER nonroot:nonroot` to the agent-board image and `USER node` to the web image. Both Dockerfiles now carry the directive (verified: agent-board Dockerfile line 31 = `USER nonroot:nonroot`; web Dockerfile line 49 = `USER node`). Re-flipped to `in_review` by orchestrator (commit `6ac27bf`).

**Code (re-verified):** `services/agent-board/internal/repo/user_story_repo.go` byte-for-byte unchanged vs main (`git diff main..HEAD -- services/agent-board/internal/repo/user_story_repo.go` produces zero output). `user_story_repo_test.go` identical to pass 1 — 21 test functions total: 5 pre-existing happy-path + 1 pre-existing rollback-on-audit-failure + 13 new UT-001..UT-013 error-branch + 1 new `_EmptyResult` for IT-001 coverage. All 13 verbatim test-function names from US026 AC present at lines 197, 221, 241, 260, 284, 309, 326, 346, 370, 395, 420, 438, 462. Pass 1's code-review checklist remains entirely clean (architecture conformance, sqlmock pattern, exhaustiveness 11/11 sites, TDG red→green→refactor sequence with `(US026)` traceability) — re-stamping it here would just duplicate pass 1's findings.
...
**Robot dryrun:** N/A — REQ006/US026 has no `tests/e2e/REQ006_*/US026_*.robot` suite (tests-only backfill task; e2e spec stub is a 176-byte placeholder).
