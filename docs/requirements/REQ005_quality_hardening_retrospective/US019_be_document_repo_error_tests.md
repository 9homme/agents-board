# US019 — Backfill 8 document-repo error-branch tests

**Story:** US019 — Backfill 14 repo error-branch tests in `internal/repo`
**Requirement:** REQ005
**Track:** BE
...
- **Out:** Project-repo tests (separate task `US019_be_project_repo_error_tests.md`); any edit to `document_repo.go` production source (§5.5 — if a test reveals a real bug, raise `ARCHITECTURE_GAP_FOUND`, do not silently fix); handler-side tests (already 100 %); `cmd/api-server` / `internal/mcp` coverage; refactoring existing happy-path tests.

## Files touched (estimated, exclusive)

- `services/agent-board/internal/repo/document_repo_test.go`

Independent of `US019_be_project_repo_error_tests.md` — different test file. Both US019 tasks parallelise cleanly. No production source touched.

## Test contract

The dev must make these tests pass (from `US019_be_unit_tests.md`, IDs assigned by tester — mapped to architecture §5.1 D1..D8):
...
- UT-US019-001 `TestDocumentRepo_CreateDocument_GenericError` — mock `ExpectQuery INSERT … WillReturnError`; asserts non-nil err, not ErrNotFound, contains "failed to create document", nil returned pointer.
- UT-US019-002 `TestDocumentRepo_GetDocument_GenericError` — mock `ExpectQuery SELECT … WillReturnError`; non-ErrNoRows path; asserts "failed to get document".
- UT-US019-003 `TestDocumentRepo_UpdateDocument_NotFound` — mock `WillReturnError(sql.ErrNoRows)`; asserts `errors.Is(err, ErrNotFound)` true.
- UT-US019-004 `TestDocumentRepo_UpdateDocument_GenericError` — mock `WillReturnError(errors.New("db down"))`; asserts "failed to update document".
- UT-US019-005 `TestDocumentRepo_DeleteDocument_GenericError` — mock `ExpectExec DELETE … WillReturnError`; asserts "failed to delete document".
- UT-US019-006 `TestDocumentRepo_ListDocuments_QueryError` — mock `ExpectQuery SELECT … WillReturnError`; asserts "failed to list documents", nil slice.
- UT-US019-007 `TestDocumentRepo_ListDocuments_ScanError` — `AddRow("doc-id", "proj-id", "Title", "Content", true, true)`: `bool` in `time.Time` column triggers `convertAssign` error; asserts "failed to scan document".
- UT-US019-008 `TestDocumentRepo_ListDocuments_RowsErr` — `RowError(0, errors.New("rows err"))` on a valid-data row; asserts "error iterating", nil slice.
...
**BE gate note:** `scripts/review/run-gate.sh be services/agent-board` currently exits with `MISSING TOOL: gosec` because US017 (soft-warn gosec/govulncheck) is still `pending`. This is an environmental constraint outside this task's scope. All logic verified via `go test ./...` directly.

## Review log

### Review pass 1 — 2026-06-03 — tech-lead (inline orchestrator review) — verdict: approved

- `cd services/agent-board && go test ./internal/repo/...`: **44 PASS** (28 pre-existing + 16 new from US019 doc+proj combined).
...
- 8 new tests (UT-US019-001..008) per arch §5.1.
- Coverage: `document_repo.go` reportedly 100% per dev (well past ≥95% DoD).
- US017 has since landed (soft-warn for gosec/govulncheck), so the BE-gate blocker the dev noted is resolved upstream.
