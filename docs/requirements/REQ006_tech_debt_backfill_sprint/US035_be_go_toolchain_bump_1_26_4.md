# US035/be_go_toolchain_bump_1_26_4
...
**Implements:** REQ006/US035 AC (all scenarios — `go.mod` bump, `Dockerfile` builder bump, `govulncheck` clean, `go test ./...` clean, `golangci-lint` clean, `go build ./...` clean for both binaries, `make e2e-up` clean), architecture §3 US035 touch row, architecture §6 (toolchain decision + verified findings + version pin + CI/Docker knock-on + govulncheck post-bump assertion), architecture D-007 (`go 1.26.4`).
...
## Scope
- **In:** Edit `services/agent-board/go.mod` line 3: `go 1.25.0` → `go 1.26.4`. Add a `toolchain go1.26.4` directive on a new line (the modern `go` + `toolchain` shape per architecture §6.2). Edit `services/agent-board/Dockerfile` line 9: `FROM golang:1.25-alpine AS build` → `FROM golang:1.26-alpine AS build` (minor-tracking alpine tag picks up latest 1.26.x at build time per architecture §6.2). Strike-through `docs/tech_debt.md` line 28 with `→ fixed in REQ006/US035`.
...
## Test contract
US035 is a pure production-code touch (no new test functions named in AC). The "tests" are existence/behaviour assertions:
...
Tester's `US035_be_unit_tests.md` will likely contain one or two UT-* IDs framed as "govulncheck-clean assertion" + "e2e-stack-builds-and-runs assertion"; no in-Go unit tests are required.
...
- **If a new finding surfaces post-bump:** raise as a follow-up story per architecture §6.5 + R-1 — do NOT silently widen US035's scope.
...
- **No story dependency.** US035 is independent of every other REQ006 story. It can land in any tick.
...
- `docs/tech_debt.md` line 28 — struck through with `→ fixed in REQ006/US035`
...
Stack: Podman machine running (`podman machine list` shows `Currently running`); `podman-compose` v1.5.0 available. The compose images (api-server, mcp-server) were built using `golang:1.26-alpine` (Dockerfile line 9 as per US035 change). All 4 containers started successfully.

**Pre-existing infrastructure issue (not caused by US035):** `make e2e-up` has two bugs that prevent it from completing on a fresh stack:
...
Both bugs were present in the Makefile before US035 (since commit `1ba4793` / REQ005/US032). US035's go toolchain bump does not cause or affect these bugs. The go.mod and Dockerfile changes have been verified correct by tech-lead (Review pass 1).
...
- `docs/tech_debt.md:28` — struck through with `→ fixed in REQ006/US035` ✓.
...
- TDG conformance — commit subjects `green: ... (US035)` + `refactor: ... (US035)` follow tdg prefix + traceability convention ✓.

**BLOCKER — IT-005 (live e2e) evidence absent.**
The task DoD (line 67) and `US035_be_unit_tests.md` IT-005 require:
...
No flakes across the three consecutive runs. The compose images were built from `golang:1.26-alpine` (the US035 Dockerfile change), confirming the new builder image works for both api-server and mcp-server binaries.

**Workaround accepted (per human direction + documented pre-existing bugs).** The exact DoD sequence `make e2e-up && make e2e-seed && make e2e-run && make e2e-down` could not run because `make e2e-up` has two pre-existing infrastructure bugs (api-server health-check circular dependency on migrations; mcp-server SSE curl hang with no `--max-time`). Both bugs predate US035 (since REQ005/US032 commit `1ba4793`) and are NOT caused by the go toolchain bump. They are filed in `docs/tech_debt.md:113` (REQ006/US035). The equivalent workaround (`podman-compose up -d` + `make e2e-seed` + `make e2e-run` ×3 + `make e2e-down`) was explicitly accepted by the human as equivalent IT-005 evidence. This is an accepted deviation, not a waived mandatory gate — the three-clean-run flake bar (architecture §10.1) was met; only the orchestration entrypoint differed.

**TDG conformance:** pass-2 rework commits `39310e6 refactor: chore: hand off go toolchain bump 1.26.4 for review (US035)` carries the `(US035)` traceability tag; `9001ddd chore: document make e2e-up health-check bugs in tech_debt.md` is a tech_debt.md documentation commit (not test/production code). The `refactor: chore:` prefix drift on hand-off commits is the same recurring housekeeping-prefix pattern already filed for REQ006/US025, US028, US031 (tech_debt.md:105,107,110) — red→green→refactor ordering is honored; tolerated as the established precedent on this REQ, not a new `changes_requested`.
