# US024 — MermaidDiagram: replace `dangerouslySetInnerHTML` with DOMParser ref-attach
...
Independent of `US024_fe_use_document_reducer_and_documents_tab.md` — different files entirely. Both US024 tasks `Blocked by:` US020's FE task per §11.4 but are NOT blocked by each other and can run in parallel.
...
- US024 tasks must show react-doctor `--diff` showing the 7 named rules cleared.
...
**Files touched:**
- `web/components/ProjectDetail/MermaidDiagram.tsx` — replaced `dangerouslySetInnerHTML` success branch with `useRef` + `useEffect` DOMParser ref-attach per architecture §11.1.2. Added `useRef<HTMLDivElement | null>(null)`, a new `useEffect([renderState])` that parses the SVG via `DOMParser('image/svg+xml')` and appends `parsed.documentElement` to the host div. Cleanup removes the appended node before strict-mode's second invocation (R7 mitigation). `try/catch` wraps the DOMParser+appendChild block per FCT-US024-004 requirement. Doc comment updated to reflect ref-attach contract.
- `web/components/ProjectDetail/MermaidDiagram.test.tsx` — added FCT-US024-001 through FCT-US024-004 (6 new test cases across 4 describe blocks, additions only — no existing assertions changed).

**Tests passing:** FCT-US024-001, FCT-US024-002, FCT-US024-003, FCT-US024-004 (+ all pre-existing FCT-US017-008, FCT-US017-009 tests). Total: 9/9 in MermaidDiagram.test.tsx; 122/122 full suite.
...
- `cd web && npm test -- --watchAll=false`: **17 suites / 130 tests pass** (includes US024 mermaid + reducer).
...
- Two unrelated pre-existing findings (line 119 `no-adjust-state-on-prop-change`, line 174 `prefer-tag-over-role`) are NOT regressions and NOT in US024 scope.
