# US013 — E2E test specification (Robot Framework)

**Owner:** tester. Implemented in `tests/e2e/REQ004_project_detail_page/US013_documents_tab.robot`.
...
### E2E-US013-001 — Documents tab auto-selects first document; click another doc updates previewer and URL
- **Tag:** US013, smoke, regression
- **Preconditions:**
  - Next.js frontend at `${WEB_BASE_URL}`, `api-server` at `${API_BASE_URL}`, MCP server running.
- **Setup (data):**
  1. Via MCP `create_project`: create project `"REQ004 US013 E2E <random>"`. Record `projectId`.
...
- **Tag:** US013, regression
- **Preconditions:** same running stack; reuse project and documents from E2E-US013-001 suite setup.
- **Steps:**
