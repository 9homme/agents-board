# US014 — E2E test specification (Robot Framework)

**Owner:** tester. Implemented in `tests/e2e/REQ004_project_detail_page/US014_markdown_mermaid.robot`.
...
XSS sanitization, error fallbacks, and individual rendering correctness assertions live at the unit layer (FCT-US014-012 through FCT-US014-015). They are NOT promoted to e2e.

## Scenarios

### E2E-US014-001 — Previewer renders syntax-highlighted code fence and mermaid SVG for a real document
- **Tag:** US014, smoke, regression
- **Preconditions:**
  - Next.js frontend at `${WEB_BASE_URL}`, `api-server` at `${API_BASE_URL}`, MCP server running.
- **Setup (data):**
  1. Via MCP `create_project`: create project `"REQ004 US014 E2E <random>"`. Record `projectId`.
