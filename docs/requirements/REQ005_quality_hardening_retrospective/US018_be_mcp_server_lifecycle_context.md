# US018 — Signal-cancellable lifecycle context for `mcp-server` DB ping
...
- **Out:** `api-server` changes (separate task `US018_be_api_server_lifecycle_context.md`); shared `internal/lifecycle/` helper (rejected by D-008); graceful HTTP shutdown (D-009); env var `DB_PING_TIMEOUT_SECONDS` (D-013); test-file `context.Background()` sweeps (D-003).
...
Independent of `US018_be_api_server_lifecycle_context.md` — different files entirely. Per D-008, NO shared helper, so no `internal/` overlap. Both US018 tasks parallelise cleanly.
...
- UT-US018-002: `TestLifecycleContext_SIGTERMCancels` (2 subtests: SIGTERM, SIGINT)
- IT-US018-002: `TestPingDB_TimeoutCancels`
- UT-US018-004: `TestMain_DeferOrder` (AST/source-position approach)
- UT-US018-005 (mcp portion): `TestMain_NoContextBackgroundAtPingSite`
