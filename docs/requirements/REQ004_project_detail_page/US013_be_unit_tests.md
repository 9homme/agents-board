# US013 — Backend unit & integration test specification
...
|---|---|---|---|---|
| List documents — happy path (multiple) | unit | UT-US013-001 | services/agent-board / internal/handler | `handler.ListProjectDocuments(c echo.Context)` |
| List documents — empty list (project exists, zero docs) | unit | UT-US013-002 | services/agent-board / internal/handler | `handler.ListProjectDocuments(c echo.Context)` |
| List documents — project not found 404 | unit | UT-US013-003 | services/agent-board / internal/handler | `handler.ListProjectDocuments(c echo.Context)` |
| List documents — 500 on project lookup failure | unit | UT-US013-004 | services/agent-board / internal/handler | `handler.ListProjectDocuments(c echo.Context)` |
| List documents — 500 on document lookup failure | unit | UT-US013-005 | services/agent-board / internal/handler | `handler.ListProjectDocuments(c echo.Context)` |
| List documents — content field absent from response | unit | UT-US013-006 | services/agent-board / internal/handler | `handler.ListProjectDocuments(c echo.Context)` |
| Get document — happy path | unit | UT-US013-007 | services/agent-board / internal/handler | `handler.GetDocument(c echo.Context)` |
| Get document — 404 not found | unit | UT-US013-008 | services/agent-board / internal/handler | `handler.GetDocument(c echo.Context)` |
| Get document — 500 internal error | unit | UT-US013-009 | services/agent-board / internal/handler | `handler.GetDocument(c echo.Context)` |
| Repo: ListDocuments orders by updated_at DESC, id DESC | unit | UT-US013-010 | services/agent-board / internal/repo | `repo.DocumentRepository.ListDocuments` |
| List documents — integration round-trip (D-006: missing project → 404 not empty list) | integration | IT-US013-001 | services/agent-board | `GET /api/v1/projects/{id}/documents` |
| List documents — integration: project exists, empty list returns `{"documents":[]}` | integration | IT-US013-002 | services/agent-board | `GET /api/v1/projects/{id}/documents` |
| List documents — integration: ordering verified | integration | IT-US013-003 | services/agent-board | `GET /api/v1/projects/{id}/documents` |
| Get document — integration round-trip | integration | IT-US013-004 | services/agent-board | `GET /api/v1/documents/{id}` |
| Get document — integration 404 | integration | IT-US013-005 | services/agent-board | `GET /api/v1/documents/{id}` |
| Routes registered on Echo | integration | IT-US013-006 | services/agent-board / cmd/api-server | `main.go` router registration |

## Unit tests

### UT-US013-001 — ListProjectDocuments handler: 200 with multiple documents
...
### UT-US013-002 — ListProjectDocuments handler: 200 empty list (project exists, no documents)
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.ListProjectDocuments(c echo.Context) error`
- **Given:**
...
### UT-US013-003 — ListProjectDocuments handler: 404 project not found (D-006)
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.ListProjectDocuments(c echo.Context) error`
- **Given:**
...
### UT-US013-004 — ListProjectDocuments handler: 500 on project lookup failure
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.ListProjectDocuments(c echo.Context) error`
- **Given:**
...
### UT-US013-005 — ListProjectDocuments handler: 500 on document list failure
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.ListProjectDocuments(c echo.Context) error`
- **Given:**
...
### UT-US013-006 — ListProjectDocuments handler: content field absent from response items
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.ListProjectDocuments(c echo.Context) error`
- **Given:**
...
### UT-US013-007 — GetDocument handler: 200 happy path
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
...
### UT-US013-008 — GetDocument handler: 404 not found
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.GetDocument(c echo.Context) error`
- **Given:**
...
### UT-US013-009 — GetDocument handler: 500 internal error
- **Service:** `services/agent-board`
- **Package under test:** `internal/handler`
- **Function under test:** `handler.GetDocument(c echo.Context) error`
- **Given:**
...
### UT-US013-010 — Repo: ListDocuments SQL orders by updated_at DESC, id DESC
- **Service:** `services/agent-board`
...
### IT-US013-001 — GET /api/v1/projects/{id}/documents — missing project returns 404 not empty list (D-006)
- **Service:** `services/agent-board`
...
### IT-US013-002 — GET /api/v1/projects/{id}/documents — project exists, zero documents returns `{"documents":[]}`
- **Service:** `services/agent-board`
- **Boundary:** handler ↔ repo ↔ real test DB
- **Setup:**
...
### IT-US013-003 — GET /api/v1/projects/{id}/documents — ordering verified
- **Service:** `services/agent-board`
- **Boundary:** handler ↔ repo ↔ real test DB
- **Setup:**
...
### IT-US013-004 — GET /api/v1/documents/{id} — found
- **Service:** `services/agent-board`
- **Boundary:** handler ↔ repo ↔ real test DB
- **Setup:**
...
### IT-US013-005 — GET /api/v1/documents/{id} — not found
- **Service:** `services/agent-board`
- **Boundary:** handler ↔ repo ↔ real test DB
...
### IT-US013-006 — Route registration smoke test for both new document routes
- **Service:** `services/agent-board`
