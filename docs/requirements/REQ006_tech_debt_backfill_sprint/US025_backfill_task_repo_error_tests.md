# US025 — Backfill `task_repo.go` error-branch tests
...
- **Scenario: `task_repo_test.go` gains the following test functions (verbatim names)**
  ...
  - **Note on UpdateTask split.** `task_repo.go:70 UpdateTask` returns both `ErrNotFound` (line 82) AND a generic error (line 84). The split into `_NotFound` + `_GenericError` mirrors REQ005/US029's `document_repo` / `project_repo` precedent.
...
- **`user_story_repo.go`** — that's US026.
- **`audit_repo.go`** — that's US027.
- **`document_repo.go` / `project_repo.go`** — shipped in REQ005/US029.
...
- **REQ005/US029 is the pattern reference.** Re-read `docs/requirements/REQ005_quality_hardening_retrospective/US029_backfill_repo_error_branch_tests.md` — this story is its lineal descendant.
