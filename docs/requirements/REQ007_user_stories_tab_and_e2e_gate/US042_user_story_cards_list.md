# US042 — User Stories tab — story cards list
...
- **Scenario: cards are keyboard- and click-activatable (prepares US043)**
  - Given cards are rendered
  - When the user clicks a card (or activates it via keyboard)
  - Then the card invokes the selection handler with that story's id (the detail view itself is US043)
...
- **Happy-path flow:**
  ...
  4. Hovering/focusing a card shows it is interactive; clicking selects it (drill-in handled by US043).
...
- **Out of UI scope:** filtering, sorting, search (D-008); pagination; the detail/drill-in view (US043); visual styling polish beyond clear card/badge legibility.

## Out of scope
- The detail panel and tasks list (US043).
...
- Keep selection state lifted so US043 can plug in the detail view without restructuring.
...
- **Spec review:** All six AC scenarios map to at least one UT-* / IT-* / FCT-* / E2E-* case.
  - Happy path (title/status/task count/desc preview): FCT-001, FCT-005, IT-001, E2E-US042-001.
  - Description truncation: FCT-001 (>80-char description) with an honest coverage exemption — CSS `line-clamp`/`text-overflow` is not computed in jsdom, so the test asserts the full text is in the DOM; visual truncation is verified at the e2e layer (E2E-US042-001 "truncated descriptions are visible"). Acceptable.
  - Empty state: FCT-002, UT-002, E2E-US042-002. Loading: FCT-003. Error: FCT-004 + IT-002/IT-003/IT-004 (404/500 paths).
