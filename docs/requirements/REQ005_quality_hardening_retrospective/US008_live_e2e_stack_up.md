# US008 — Live e2e stack-up: docker-compose + Makefile + seeded Postgres for end-to-end Robot runs
...
- **Scenario: `make e2e-run` executes Robot suites against the live stack**
  ...
  - When `make e2e-run` is invoked (optionally `make e2e-run REQ=REQ001 US=US001` for narrowing)
...
- **Scenario: existing per-REQ tests/e2e suites still run**
  ...
  - When `make e2e-run REQ=REQ001 US=US001` is invoked
...
- **Operator flow:** orchestrator in Phase 3c spawns `make e2e-run REQ=REQ005 US=US008` (or whatever) after BE / FE test captures, parses the Robot output for E2E-* IDs, writes them into the per-story `US###_test_report.md`.
...
## Dependencies
- Soft: US001 + US002 + US003 should land first so the per-track gates can run cleanly while this story is in development. Not a hard blocker.
