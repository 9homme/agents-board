# US038 — Consolidate dev workflow into the Makefile; retire `startup.sh` / `shutdown.sh`
...
- **Scenario: `make dev-up` starts the three native processes**
  ...
  - And both `api-server` and `mcp-server` are invoked with `DATABASE_URL=...` set in their environment (per US034's single-var contract)
  - And `DB_URL` is NOT set anywhere in the `dev-up` recipe (validates US034 alignment)
...
- **Scenario: closes tech-debt finding for `PG_CONN` overridability**
  ...
  - Then `docs/tech_debt.md` line 86 is struck through with `→ fixed in REQ006/US038 (absorbed from former US011)`
...
- **Scenario: `DATABASE_URL` is the only DB env var used in any new `dev-*` recipe**
  ...
  - And `DATABASE_URL` is the sole DB env var passed to `api-server` and `mcp-server` (validates alignment with REQ006/US034)
...
- **Scenario: documentation and agent definitions updated**
  ...
  - And `docs/tech_debt.md` — if it contains any line referencing `startup.sh` / `shutdown.sh` directly — is struck through with `→ fixed in REQ006/US038`
...
- **Zero `DB_URL` in any recipe.** Validates US034 alignment. Both `api-server` and `mcp-server` see only `DATABASE_URL`.
...
- **Sequence with US034:** US038 SHOULD ship **before or in the same merge as** US034. **Rationale:** US034 introduces a hard-fail when `DB_URL` is set, which would break the existing `startup.sh` workflow until US038 deletes it (because `startup.sh` today references `DB_URL`). No hard `Blocked by` link — tech-lead may pair both PRs in one merge if simpler, or sequence US038 → US034 in two PRs.
