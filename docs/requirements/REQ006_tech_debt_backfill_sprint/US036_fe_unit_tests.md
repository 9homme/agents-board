# US036 — Frontend component test specification
...
|---|---|---|---|
| Click non-active tab fires `onTabChange` | FCT-US036-001 | `TabSwitcher` | callback called once with clicked id; controlled component does not mutate `activeTab` |
| `ArrowRight` moves focus forward and fires `onTabChange` | FCT-US036-002 | `TabSwitcher` | focus on next tab, callback called with next tab id |
| `ArrowRight` from last tab wraps to first | FCT-US036-003 | `TabSwitcher` | modulo wrap; callback called with first tab id |
| `ArrowLeft` moves focus backward and fires `onTabChange` | FCT-US036-004 | `TabSwitcher` | focus on previous tab, callback called with previous tab id |
| `ArrowLeft` from first tab wraps to last | FCT-US036-005 | `TabSwitcher` | modulo wrap; callback called with last tab id |
| `Enter` activates focused tab; calls `preventDefault` | FCT-US036-006 | `TabSwitcher` | callback called for currently focused tab |
| `Space` activates focused tab; calls `preventDefault` | FCT-US036-007 | `TabSwitcher` | callback called for currently focused tab |
| `aria-selected` reflects `activeTab` prop | FCT-US036-008 | `TabSwitcher` | correct `aria-selected` on each tab button |
| Roving `tabIndex` reflects `activeTab` prop | FCT-US036-009 | `TabSwitcher` | `tabIndex={0}` on active, `tabIndex={-1}` on inactive |
| Prop-driven `activeTab` change re-renders; no `onTabChange` fired | FCT-US036-010 | `TabSwitcher` | new active tab rendered; callback NOT called by re-render |
| Tablist semantics present | FCT-US036-011 | `TabSwitcher` | `role="tablist"`, `aria-label`, `aria-controls`, `id` attributes |
| Unrelated keys do not fire `onTabChange` or `preventDefault` | FCT-US036-012 | `TabSwitcher` | `onTabChange` not called; `preventDefault` not called |

## Component tests

### FCT-US036-001 — Clicking non-active tab fires `onTabChange` with clicked id
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-001

---

### FCT-US036-002 — `ArrowRight` moves focus forward and fires `onTabChange`
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-002
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-003; architecture.md §7.1 modulo arithmetic

---

### FCT-US036-004 — `ArrowLeft` moves focus backward and fires `onTabChange`
- **Component:** `TabSwitcher`
- **Render with:** `activeTab="user-stories"`
- **User interaction:**
  ```tsx
...
- **Expect:**
  ...
  - Focus is on the Documents tab
- **Architecture cite:** architecture.md §7.2 FCT-US036-004
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-005
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-006

---

### FCT-US036-007 — `Space` activates focused tab; calls `preventDefault`
- **Component:** `TabSwitcher`
- **Render with:** same as FCT-US036-006
- **User interaction:**
  ```tsx
  await user.click(screen.getByRole('tab', { name: /documents/i }))
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-007

---

### FCT-US036-008 — `aria-selected` reflects `activeTab` prop
- **Component:** `TabSwitcher`
- **Render with:** `activeTab="documents"`
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-008; architecture.md §7.1 ARIA attributes

---

### FCT-US036-009 — Roving `tabIndex` reflects `activeTab` prop
- **Component:** `TabSwitcher`
- **Render with:** `activeTab="documents"`
- **Expect:**
  - Documents tab has `tabIndex={0}`
  - User Stories tab has `tabIndex={-1}`
- **Architecture cite:** architecture.md §7.2 FCT-US036-009; architecture.md §7.1 roving tabindex

---

### FCT-US036-010 — Prop-driven `activeTab` change re-renders; no `onTabChange` fired
- **Component:** `TabSwitcher`
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-010

---

### FCT-US036-011 — Tablist semantics are present
- **Component:** `TabSwitcher`
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-011; architecture.md §7.1 ARIA attributes

---

### FCT-US036-012 — Unrelated keys do not fire `onTabChange` or `preventDefault`
- **Component:** `TabSwitcher`
...
- **Architecture cite:** architecture.md §7.2 FCT-US036-012; architecture.md §7.1 NOT IMPLEMENTED list
