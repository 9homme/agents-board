...
- 2026-06-05 — services/agent-board/Dockerfile:31 + web/Dockerfile:48 — semgrep `dockerfile.security.missing-user.missing-user` finding (no `USER non-root` line; containers run as root) blocks every `scripts/review/run-gate.sh cross` invocation; pre-existing on `main`, already approved-around on REQ006/US001 and US002 — add `USER non-root` to both Dockerfiles in a follow-up REQ so cross-gate is meaningful again — REQ006/US013/fe_tabswitcher_coverage_backfill
