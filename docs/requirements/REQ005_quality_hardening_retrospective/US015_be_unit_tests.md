# US015 — Backend unit & integration test specification
...
|---|---|---|---|---|
| Non-TTY failing `run_check` prints failure body, no `invalid option` | integration | IT-US015-001 | `scripts/review/run-gate.sh` | `run_check` failure output in non-TTY mode |
| Non-TTY failing `run_check_warn` prints warning body, no `invalid option` | integration | IT-US015-002 | `scripts/review/run-gate.sh` | `run_check_warn` failure output in non-TTY mode |
| PASS path: no failure banner, exit 0 | integration | IT-US015-003 | `scripts/review/run-gate.sh` | full gate PASS path smoke |
| TTY run prints YELLOW banner (no regression) | integration | IT-US015-004 | `scripts/review/run-gate.sh` | `run_check` failure output in TTY-like mode |
...
### IT-US015-001 — Non-TTY failing `run_check` shows failure body without `invalid option`
...
- **Architecture cite:** architecture §2 US015 row — `printf` at lines 58 and 71 of `run-gate.sh` must be prefixed with `--`.

### IT-US015-002 — Non-TTY failing `run_check_warn` shows warning body without `invalid option`
...
- **Architecture cite:** architecture §2 US015 row — line 71 (`run_check_warn` output print block) receives the same `--` fix.

### IT-US015-003 — PASS path smoke: no failure banner, exit 0
...
- **Architecture cite:** US015 AC "Scenario: no regression on the PASS path".

### IT-US015-004 — TTY-like mode: YELLOW banner still rendered (no regression)
...
- **Setup:**
  ...
  - Inject a failing step as in IT-US015-001.
...
- **Then:**
  ...
  - No `printf: --: invalid option` line appears.
- **Notes:** If the test environment cannot produce a pseudo-TTY, this test may be marked as a manual verification step in the test report; that is acceptable provided IT-US015-001 and IT-US015-002 are automated.
- **Architecture cite:** US015 AC "Scenario: TTY run unchanged".
