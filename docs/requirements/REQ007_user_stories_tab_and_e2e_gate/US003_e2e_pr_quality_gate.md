# US003 — GitHub Actions e2e PR quality gate
...
## Dependencies
- **US002** — CI relies on a working `make e2e-up`/`e2e-seed`/`e2e-run` chain; the health-check bugs must be fixed first or the workflow will time out/hang exactly as the local stack does.
