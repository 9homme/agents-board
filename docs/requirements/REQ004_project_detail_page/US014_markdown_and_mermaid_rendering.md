# US014 — Markdown and mermaid rendering in the previewer
...
> Scope note: this story upgrades the previewer's internals only. It does NOT touch the sidebar, navigation, fetching, error/loading states, or routing — those are owned by US013. From the user's point of view, US014 changes "what the content area looks like" once a document has loaded.
...
- **Entry points:** Same as US013 — user is on the Documents tab and a document is selected. This story changes only what the user sees inside the previewer pane.

- **Happy-path flow (visual upgrade only):**
  1. User selects a document (US013 behavior unchanged).
  2. Previewer fetches content (US013 behavior unchanged).
  3. **NEW in US014:** once content arrives, the previewer renders it as styled markdown with mermaid diagrams and syntax-highlighted code, rather than as plain text / `<pre>`.
...
- **Loading / error states:**
  - Loading state for the content fetch itself is unchanged from US013.
...
## Dependencies
- **US013** — provides the previewer container, the document fetch, the selection/loading/error states. US014 replaces the previewer's internal rendering only.
- **No new backend endpoints.** US014 is FE-only (mermaid + markdown rendering is a client-side concern; mermaid renders in the browser and must run via dynamic import to keep the bundle small).
...
**Spec review (`US014_fe_unit_tests.md` + `US014_e2e_tests.md`):**
- Every AC scenario maps to at least one FCT-* / E2E-* case:
  - "Headings render as headings" → FCT-US014-001.
  - "Paragraphs, bold, italic, and inline code render" → FCT-US014-002.
  - "Lists render (ordered + unordered + task lists)" → FCT-US014-003 (asserts disabled checkbox state for `[ ]` and `[x]`).
  - "Tables render" → FCT-US014-004 (asserts `<table>` + `<thead>` + `<tbody>` + cell counts via roles).
  - "Links render and are safe" → FCT-US014-005 (safe href + `target=_blank` + `rel=noopener noreferrer`; `javascript:` URI sanitization). Reinforced by FCT-US014-013.
  - "Code fences render syntax-highlighted" → FCT-US014-006 (asserts `language-go` + `hljs` classes AND tokenised `<span>` children — proves the highlighter actually ran, not just class noise). Negative branch "no language tag → plain `<pre><code>` without throwing" → FCT-US014-007.
  - "Mermaid diagrams render as SVG" → FCT-US014-008 (asserts `<svg>` present AND raw mermaid source text not in the rendered DOM).
  - "Invalid mermaid source does not crash the page" → FCT-US014-009 (asserts `role="alert"` fallback + raw source in `<pre>` + no exception propagates).
  - "Switching documents re-renders mermaid for the new content" → FCT-US014-011 (asserts old SVG gone after `key` change, new SVG present — proves the `key={document.id}` invariant from US013 still holds after the markdown swap).
  - "XSS — script tags in content are not executed" → FCT-US014-012 (real `<script>alert(1)</script>` payload; asserts zero `<script>` elements AND `window.alert` spy not called AND safe paragraph still rendered). Reinforced by FCT-US014-014 (`<img onerror=...>` stripped — also asserts `window.alert` spy not called).
- Defence-in-depth coverage beyond the bare AC:
  - FCT-US014-010 enforces the lazy-load contract — `mermaid.render` must NOT be called for non-mermaid documents (architecture D-004 / bundle-size posture).
  - FCT-US014-015 covers the `MarkdownErrorBoundary` — pathological renderer throw shows "Failed to render document" instead of blanking the page.
...
**Result review (`US014_test_report.md`):**
- Backend: N/A (US014 is FE-only). Full BE suite remains green from the US013 capture — no regression risk from this FE-only story.
- Frontend: US014-scoped command `cd web && npm test -- --watchAll=false --forceExit --testPathPatterns='MarkdownRenderer|Mermaid|DocumentPreviewer'` → **Test Suites: 3 passed, 3 total; Tests: 28 passed, 28 total**. Full FE suite **107/107**. All 15 FCT-US014-001..015 PASS. No skips, no `.skip`, no `t.Skip`. The mermaid AC coverage is correctly distributed across `MermaidDiagram.test.tsx` (component-level) and `MarkdownRenderer.test.tsx` (code-override routing).
- Aux gates: `npm run typecheck` clean; `npm run lint -- --max-warnings=0` clean (`ESLint: No issues found`); `bash scripts/review/run-gate.sh cross` → `REVIEW GATE: PASS` (semgrep + gitleaks). The pre-existing `run-gate.sh fe` hang at `npm test --watchAll=false` due to MSW open handles is the same posture as US012/US013 — both FE tech-lead reviews verified the constituent checks individually.
- E2E: `robot --dryrun --include US014 tests/e2e/REQ004_project_detail_page/` → 1/1 parsed cleanly (US012 import-path fix `31f162d` unblocked all REQ004 suites). E2E-US014-001 live execution NOT performed against a real `web` + `api-server` + seeded-DB stack — accepted as a release-gate item, matching the US012 pass 2 and US013 pass 2 sign-off posture.
...
- Live e2e for E2E-US014-001 is a release-gate item (same as US012 + US013) — requires standing up `cd web && npm run dev` + `cd services/agent-board && go run ./cmd/api-server` against a seeded DB. Tracked here, not blocking sign-off.
