# US022 — E2E test specification

**Owner:** tester. Implemented in `tests/e2e/REQ005_quality_hardening_retrospective/US022_stack_smoke.robot`.

## Why e2e

US022 delivers the live e2e stack itself (`docker-compose.yml`, `Makefile`, `Dockerfile`s, seed SQL). The primary validation of US022 is therefore a Robot Framework smoke test that:
...
The unit/integration-level assertions (Makefile dry-runs, SQL parse checks — IT-US022-001 through IT-US022-007) verify correctness of the infrastructure files themselves. The e2e layer verifies that the assembled stack actually works end-to-end once all the pieces are in place.

This is the one story where e2e is definitionally necessary — without a passing e2e test, US022's core claim ("the stack comes up and Robot runs against it") cannot be verified at any lower layer.
...
- **Tag:** US022, smoke
- **Preconditions:** same as E2E-US022-001. `WEB_BASE_URL` = `http://localhost:3000`.
...
### E2E-US022-003 — Existing Robot suite executes against live stack without connection error

- **Tag:** US022, smoke, regression
...
- **Notes:** This scenario is NOT run by Robot Framework itself — it is executed by the `make e2e-run` target and its result is captured in the test report. The tester documents in the test report:
  ...
  - If tests fail due to stale seed data or changed API shapes from prior REQs, that is a per-REQ follow-up — NOT a US022 failure. US022's AC only requires that the tests CAN run (connection-refused errors are gone).
...
- **Architecture cite:** architecture §6.6 — existing `.robot` files run via `make e2e-run`; US022 AC "Scenario: existing per-REQ tests/e2e suites still run".
