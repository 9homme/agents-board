...
**Note — empty-state coverage:** A standalone E2E-US005-003 scenario for the empty-state is not carried as a separate test case. The empty-state assertion ("No tasks for this story.") is exercised as step 3a of this scenario. Combined with FCT-002 (component-level empty state) and IT-004 (backend returns empty task list), the AC is fully covered without an additional e2e case that would duplicate the same seed data and drawer-open steps.

## Spec change log

### Revision 1 — 2026-06-09 — driver: po-ba sign-off pass 1

- changed E2E-US005-002 — updated close step from "Click the close (X) button" to "Press the Escape key"; added rationale note (Escape is more reliable in headless Playwright; X-button path proven at FCT-003). Updated precondition wording to make explicit that Story 2 has 0 tasks.
- changed E2E-US005-002 — added step 3a as an explicit sub-assertion for the empty-state ("No tasks for this story.") that was previously implicit in the robot file but undocumented in the spec.
- removed E2E-US005-003 — standalone empty-state scenario removed from the spec. The AC is fully covered by step 3a of E2E-US005-002 (e2e layer) + FCT-002 (FE component layer) + IT-004 (BE integration layer). A separate test case would duplicate seed data and drawer-open steps without adding coverage. Coverage exemption note added inline.
