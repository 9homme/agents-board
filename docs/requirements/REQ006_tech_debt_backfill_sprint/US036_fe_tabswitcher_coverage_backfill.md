# US036/fe_tabswitcher_coverage_backfill
...
**Implements:** REQ006/US036 AC (all 12 scenarios — click, ArrowRight/Left + wrap, Enter, Space, aria-selected, roving tabIndex, prop-driven override, tablist semantics, unrelated-key no-op), architecture §3 US036 touch row, architecture §7 (component capability confirmation + 12 FCT-* IDs + 80% coverage target + tooling pin), architecture D-009 (≥80% across stmts/branches/lines/functions).
...
## Test contract
Dev makes the following FCT-* IDs pass (architecture §7.2 — verbatim, 1:1 with US036 AC scenarios):
- **FCT-US036-001** — clicking non-active tab fires `onTabChange` with clicked id; component does NOT internally mutate `activeTab`.
- **FCT-US036-002** — `ArrowRight` moves focus forward + fires `onTabChange`.
- **FCT-US036-003** — `ArrowRight` from last tab wraps to first.
- **FCT-US036-004** — `ArrowLeft` moves focus backward + fires `onTabChange`.
- **FCT-US036-005** — `ArrowLeft` from first tab wraps to last.
- **FCT-US036-006** — `Enter` activates focused tab; calls `preventDefault`.
- **FCT-US036-007** — `Space` activates focused tab; calls `preventDefault`.
- **FCT-US036-008** — `aria-selected` per tab reflects `activeTab` prop.
- **FCT-US036-009** — Roving `tabIndex` per tab reflects `activeTab` prop.
- **FCT-US036-010** — Prop-driven `activeTab` change re-renders with new active tab AND does NOT fire `onTabChange`.
- **FCT-US036-011** — Tablist semantics (`role="tablist"`, `aria-label`, `aria-controls`, `id`).
- **FCT-US036-012** — Unrelated keys do NOT fire `onTabChange` AND do NOT call `preventDefault`.

Tester's `US036_fe_unit_tests.md` FCT-* IDs map 1:1 onto the architecture §7.2 list.
...
- `docs/tech_debt.md` line 75 strike-through applied with `→ fixed in REQ006/US036`.
...
## Notes
- **No FE/BE pairing.** US036 stands alone; no API contract is touched. The BE-side state of REQ006 is irrelevant to this story's correctness.

### Hand-off notes (fe-dev-2026-06-05T00:00:00Z-a121)

**Files touched:**
- `web/components/ProjectDetail/TabSwitcher.test.tsx` — edited: added 12 FCT-US036-* test cases; `@testing-library/user-event` imported
- `docs/tech_debt.md` — line 75 struck through with `→ fixed in REQ006/US036`
- `docs/requirements/REQ006_tech_debt_backfill_sprint/US036_fe_tabswitcher_coverage_backfill.md` — status updated
- `docs/requirements/REQ006_tech_debt_backfill_sprint/US036_fe_unit_tests.md` — added to worktree (read-only reference)

**Tests added:** 12 FCT-US036-* cases (FCT-US036-001 through FCT-US036-012). Total suite: 16 TabSwitcher tests, 142 total.
...
**TDG cycle commits:**
- `red:` — 12 FCT-US036-* test cases written
...
- All 12 FCT-US036-* test cases present at `web/components/ProjectDetail/TabSwitcher.test.tsx:62-233`, naming matches §7.2 verbatim (FCT-US036-001…012).
...
- Architecture-excluded keys (`Home`, `End`, `Escape`, `Tab`) are NOT tested as behaviour-fires — only `FCT-US036-012` asserts the no-op on unrelated keys (`Escape`, `a`, `Tab`). Conforms to §7.1 NOT IMPLEMENTED list.
- Architecture §10.2 explicitly waives live e2e for this component-only story: "US036 is component-level only; it does not require an e2e run." Confirmed — no e2e run performed.

**Test contract:**
- All 12 FCT-* IDs in `US036_fe_unit_tests.md` are implemented; the existing 4 FCT-US025-* tests are preserved.
- TabSwitcher suite: 16/16 passing. Full FE suite: 142/142 passing.

**TDD honesty (TDG):**
- Commit sequence on dev branch `worktree-agent-a121284da93c79bfa`: `2af8d73 red:` → `eb9396b green:` → `0d2c5fe refactor: chore:` → `dd998f4 refactor: chore: hand off`. All commits end with `(US036)` traceability tag and follow red → green → refactor ordering. Conforms.
...
- Verified at `docs/tech_debt.md:75` — strike-through applied with `→ fixed in REQ006/US036` suffix. Conforms.

**Spec exhaustiveness (anti-REQ005/US029 branch-count check):**
...
- `scripts/review/run-gate.sh cross`: emits `REVIEW GATE: FAIL (1 check(s))` — semgrep `dockerfile.security.missing-user.missing-user` finding on `services/agent-board/Dockerfile:31` and `web/Dockerfile:48`. **Verified pre-existing on `main`:** `git diff main -- services/agent-board/Dockerfile web/Dockerfile` → 0 lines. Same finding flagged identically on sibling tasks `US025_be_task_repo_error_tests.md` and `US026_be_user_story_repo_error_tests.md` and approved-around there (cross-gate FAIL on out-of-scope, pre-existing Dockerfile files; same repo state). gitleaks: PASS.
...
- Sibling-task precedent in this same REQ (US025, US026, both already merged to `main`) documents the identical pre-existing FAIL and approved-around it.
- Approving here follows established REQ006 norm; the dockerfile-USER finding is a **repo-wide tech-debt item not tied to US036** and is being filed below for a dedicated follow-up (so it stops blocking every review).

**Tech-debt filed:**
- 2026-06-05 — services/agent-board/Dockerfile:31 + web/Dockerfile:48 — semgrep `dockerfile.security.missing-user.missing-user` finding (containers run as root) blocks every `scripts/review/run-gate.sh cross` invocation; pre-existing across all REQ006 reviews; add `USER non-root` line OR exempt in a semgrep baseline so cross-gate is meaningful again — REQ006/US036 (filed during review pass 1).
