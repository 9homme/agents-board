# US017 — Backend unit & integration test specification
...
|---|---|---|---|---|
| BE gate completes when `gosec` is absent | integration | IT-US017-001 | `scripts/review/run-gate.sh` | soft-warn WARN line emitted; gate does not exit 2; continues |
| BE gate completes when `govulncheck` is absent | integration | IT-US017-002 | `scripts/review/run-gate.sh` | soft-warn WARN line emitted; gate does not exit 2; continues |
| When both ARE installed, behaviour unchanged (no spurious WARN) | integration | IT-US017-003 | `scripts/review/run-gate.sh` | `gosec` and `govulncheck` run_check steps execute; no WARN line |
| `go` and `golangci-lint` remain hard-required (exit 2) | integration | IT-US017-004 | `scripts/review/run-gate.sh` | missing `go` causes exit 2; missing `golangci-lint` causes exit 2 |
| Cross and FE gates unaffected | integration | IT-US017-005 | `scripts/review/run-gate.sh` | soft-warn only in `gate_be()`; cross/fe tracks unchanged |
| README documents hard vs soft-warn tools | integration | IT-US017-006 | `scripts/review/README.md` | file contains required subsection content |

## Integration tests

### IT-US017-001 — BE gate soft-warns when `gosec` is absent
...
- **Architecture cite:** architecture §2 US017 row — inline `command -v` check with WARN print + skip for `gosec`.

### IT-US017-002 — BE gate soft-warns when `govulncheck` is absent
...
- **Architecture cite:** architecture §2 US017 row; US017 AC "Scenario: BE gate completes when `govulncheck` is absent".

### IT-US017-003 — When both tools ARE installed, no WARN line appears
...
- **Architecture cite:** US017 AC "Scenario: when both binaries ARE installed, behaviour unchanged".

### IT-US017-004 — `go` and `golangci-lint` remain hard-required (exit 2)
...
- **Architecture cite:** US017 AC "Scenario: `go` and `golangci-lint` remain hard-required".

### IT-US017-005 — Cross and FE gate tracks unaffected
...
- **Architecture cite:** US017 AC "Scenario: cross-cutting and FE gates unaffected".

### IT-US017-006 — README documents hard vs soft-warn distinction
...
- **Architecture cite:** architecture §2 US017 row — README update; US017 AC "Scenario: README documents the substitution".
