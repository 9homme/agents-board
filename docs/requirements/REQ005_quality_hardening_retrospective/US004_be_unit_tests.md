...
|---|---|---|---|---|
| SIGTERM cancels lifecycle context (api-server) | unit | UT-US004-001 | services/agent-board / cmd/api-server | `signal.NotifyContext` cancellation on SIGTERM |
| SIGTERM cancels lifecycle context (mcp-server) | unit | UT-US004-002 | services/agent-board / cmd/mcp-server | `signal.NotifyContext` cancellation on SIGTERM |
| DB ping respects 5 s timeout (api-server) | integration | IT-US004-001 | services/agent-board / cmd/api-server | `db.PingContext` times out with `context.DeadlineExceeded` |
| DB ping respects 5 s timeout (mcp-server) | integration | IT-US004-002 | services/agent-board / cmd/mcp-server | `db.PingContext` times out with `context.DeadlineExceeded` |
| Defer order: cancel before stop before db.Close (api-server) | unit | UT-US004-003 | services/agent-board / cmd/api-server | deferred call order verified via call-order recorder |
| Defer order: cancel before stop before db.Close (mcp-server) | unit | UT-US004-004 | services/agent-board / cmd/mcp-server | deferred call order verified via call-order recorder |
| No `context.Background()` at the two named sites | unit | UT-US004-005 | services/agent-board / cmd/{api-server,mcp-server} | static grep assertion |
| Happy path: PingContext succeeds, no error returned (api-server) | integration | IT-US004-003 | services/agent-board / cmd/api-server | `run()` proceeds past ping with healthy DB mock |

## Unit tests

### UT-US004-001 — SIGTERM cancels lifecycle context (api-server)
...
- **Given/When/Then:** identical to UT-US004-001 but instantiated in the `cmd/mcp-server` package.
...
- **Architecture cite:** architecture §3.4 — "The two files do NOT share a helper"; §2 US004 row for mcp-server.

### UT-US004-003 — Defer order: cancel → stop → db.Close (api-server)
...
- **Function under test:** `run()` deferred cleanup order
- **Given/When/Then:** identical pattern to UT-US004-003, applied to `cmd/mcp-server/main_test.go`.
- **Architecture cite:** architecture §3.5; §2 US004 row for mcp-server.

### UT-US004-005 — No `context.Background()` at the two named production sites
...
- **Architecture cite:** US004 AC "Scenario: no `context.Background()` remains in production code at the two named sites".

## Integration tests

### IT-US004-001 — DB ping times out with `context.DeadlineExceeded` (api-server)
...
- **Service:** `services/agent-board`
- **Package under test:** `cmd/mcp-server`
- **Boundary:** same as IT-US004-001
- **Setup/When/Then:** identical to IT-US004-001 but instantiated in `cmd/mcp-server/main_test.go`.
- **Architecture cite:** architecture §2 US004 row (mcp-server line at `main.go:37`).

### IT-US004-003 — Happy path: PingContext succeeds, `run()` proceeds (api-server)
...
- **Architecture cite:** US004 AC "Scenario: happy path unchanged"; architecture §3.4.
