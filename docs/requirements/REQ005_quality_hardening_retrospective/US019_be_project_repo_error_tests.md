# US019 — Backfill 8 project-repo error-branch tests
...
- **Out:** Document-repo tests (separate task `US019_be_document_repo_error_tests.md`); any edit to `project_repo.go` production source (§5.5); handler-side tests; `cmd/api-server` / `internal/mcp` coverage; refactoring existing happy-path tests.

## Files touched (estimated, exclusive)

- `services/agent-board/internal/repo/project_repo_test.go`

Independent of `US019_be_document_repo_error_tests.md`. Both US019 tasks parallelise cleanly.

## Test contract

The dev must make these tests pass (from `US019_be_unit_tests.md`, IDs assigned by tester — mapped to architecture §5.2 P1..P8):
...
**Tests added (8 new):**
- UT-US019-009 `TestProjectRepo_CreateProject_GenericError` — P1
- UT-US019-010 `TestProjectRepo_GetProject_GenericError` — P2
- UT-US019-011 `TestProjectRepo_UpdateProject_NotFound` — P3
- UT-US019-012 `TestProjectRepo_UpdateProject_GenericError` — P4
- UT-US019-013 `TestProjectRepo_DeleteProject_GenericError` — P5
- UT-US019-014 `TestProjectRepo_ListProjects_QueryError` — P6
- UT-US019-015 `TestProjectRepo_ListProjects_ScanError` — P7
- UT-US019-016 `TestProjectRepo_ListProjects_RowsErr` — P8
...
- `cd services/agent-board && go test ./internal/repo/...`: **44 PASS** (28 pre-existing + 16 new across US019 doc+proj).
...
- 8 new tests (UT-US019-009..016) per arch §5.2.
...
- 2026-06-03 — services/agent-board/cmd/mcp-server/sse.go,message.go — pre-existing govulncheck finding on stdlib crypto/x509 (transitive via Go stdlib) — bump Go toolchain or pin a fix when a runtime upgrade is available — REQ005/US019/be_project_repo_error_tests
