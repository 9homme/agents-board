...
- **Scenario: both binaries accept only `DATABASE_URL`**
  ...
  - And **both** binaries refuse to start (log a fatal error and exit non-zero) when only `DB_URL` is set, with the exact message `"DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US010)"` (locked in `architecture.md` §5.4)
...
- **Scenario: deprecated `DB_URL` is rejected at startup with a clear actionable error**
  ...
  - And the fatal log line MUST contain the substring `"DB_URL is no longer supported"` AND the substring `"rename to DATABASE_URL"` AND the substring `"REQ006/US010"` (so a grep finds the migration ticket)
  - Given the same deployment but with BOTH `DB_URL` AND `DATABASE_URL` set
  - When the binary starts
  - Then it MUST exit non-zero **before any DB ping attempt**
  - And the fatal log line MUST contain the substring `"remove DB_URL"` AND the substring `"DATABASE_URL is the sole accepted name"` AND the substring `"REQ006/US010"`
  - **Exact wording (per `architecture.md` §5.4):**
    - DB_URL-only case: `"DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US010)"`
    - Both-set case: `"DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US010)"`
...
- **Scenario: existing happy-path behaviour is preserved (api-server) and intentionally broken (mcp-server passing `DB_URL`)**
  ...
  - Then it now hard-fails at startup with the §5.4 rename-instruction error — this is the deliberate breaking change introduced by REQ006/US010 per `architecture.md` §5.1 / D-006
...
- **Scenario: env-var resolution is unit-tested**
  ...
  - Then unit tests in `services/agent-board/internal/config/dburl_test.go` cover all four env-state combinations:
    ...
    2. `TestResolveDBURL_OnlyDBURLSet_Errors` — `DB_URL` set, `DATABASE_URL` unset → returns `("", err)` where `err.Error()` matches the §5.4 wording `"DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US010)"`
    3. `TestResolveDBURL_BothSet_Errors` — both set → returns `("", err)` where `err.Error()` matches the §5.4 wording `"DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US010)"`
...
- **Scenario: Docker/compose configuration updated consistently**
  ...
  - And the pre-existing-inconsistency comment block (REQ005 lines 46–47) is replaced with the one-liner `# Standardised on DATABASE_URL per REQ006/US010 (D-006). DB_URL is rejected at startup.` (per `architecture.md` §5.5)
...
- **Scenario: documentation updated**
  ...
  - When this story is complete
  - Then any such documentation is updated to say `"DATABASE_URL only; DB_URL is rejected at startup as of REQ006/US010"`
  - And `docs/tech_debt.md` line 97 (the env-var-harmonisation finding) is struck through with `→ fixed in REQ006/US010 (single-var contract; DB_URL now rejected at startup)`

- **Scenario: closes REQ005 architecture OQ-7**
  - Given the architecture OQ-7 about env-var harmonisation was deferred from REQ005
  - When REQ006/US010 is `done`
...
- **Startup error lines — THREE distinct, operator-actionable messages** (the three §5.4 cases):
  - Only `DB_URL` set: `"DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US010)"` — names the env var to remove, the env var to set, and the migration ticket so an operator can grep.
  - Both `DB_URL` and `DATABASE_URL` set: `"DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US010)"` — explicitly tells the operator to remove the legacy var so intent is unambiguous.
...
- **Shared helper, locked.** `architecture.md` §5.2 locks the shared helper in new package `services/agent-board/internal/config` (file `dburl.go`). The "duplicate into both `main.go`" option is dropped — US010's AC requires unit tests for the helper at ≥95% coverage, and a package gives a single clean test surface rather than two duplicate test files.
...
- **Why both names historically.** `api-server` followed Heroku/12-factor convention (`DATABASE_URL`); `mcp-server` was written later by a contributor who used `DB_URL`. REQ006/US010 ends this divergence by deprecating `DB_URL` outright.
...
- **Closes tech-debt.** Strike through `docs/tech_debt.md` line 97 in the same commit that ships US010 (or in the sign-off commit) with the wording `→ fixed in REQ006/US010 (single-var contract; DB_URL now rejected at startup)` — see "Scenario: documentation updated" above.
