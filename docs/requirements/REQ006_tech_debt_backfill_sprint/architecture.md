...
- **§3 File-level touch map (US025..US038)** — the authoritative scoping table for tech-lead + devs.
...
- **§5 US034 env-var harmonisation contract** — `DATABASE_URL` wins, helper shape, log-line wording.
- **§6 US035 Go toolchain bump** — pinned `go 1.26.4` based on live `govulncheck` run.
- **§7 US036 FE TabSwitcher scope** — confirms component capabilities and pins FCT-* IDs the tester will produce.
- **§8 US032 message.go test harness shape** — httptest + Echo for `HandleMessage`, direct call for the two helpers.
- **§9 US037 ADR-001 — MCP-only-writes (inline ADR text).** This IS the deliverable of US037.
- **§10 Skill / hook usage** — TDG + react-doctor + live-e2e + 3-clean-run flake check, with the US037 documentation-only carve-out.
...
2. **US034 env-var contract: `DATABASE_URL` is the SOLE accepted env var; `DB_URL` is REJECTED at startup with a fatal error** (D-006, revised per human direction at Phase 1 HARD STOP). There is no precedence rule because there is only one variable. Helper still lives in new `services/agent-board/internal/config/dburl.go` (kept for the unit-test surface US034's AC requires; ≥95% per-package coverage). Helper API simplifies to `ResolveDBURL() (url string, err error)` — returns the `DATABASE_URL` value on the happy path; returns a clear, operator-actionable error if `DB_URL` is set (telling the operator to rename), if both are set (telling the operator to remove `DB_URL`), or if neither is set. Startup log line: just `"db config: using DATABASE_URL"` (single happy-path variant — the both-set log variant is dropped because both-set is now a fatal error, not a precedence resolution). `docker-compose.yml` switches mcp-server's env block to `DATABASE_URL` in the same commit; api-server is already on `DATABASE_URL`. Migration impact is explicit and intentional: any external operator (Helm chart, custom compose override, CI script) still passing `DB_URL` to mcp-server MUST rename — there is no quiet upgrade path. This is the deliberate cost of the hard-fail choice (avoids silent dual-source-of-truth bugs during the migration).
3. **US035 Go toolchain target: `go 1.26.4`** (D-007). Live `govulncheck` against the host `go1.26.3` showed TWO real findings (`crypto/x509` GO-2026-5037 + `net/textproto` GO-2026-5039), both fixed in `1.26.4`. `go.mod` updated to `go 1.26.4` with `toolchain go1.26.4`; `Dockerfile:9` builder updated to `golang:1.26-alpine` (the alpine tag tracks the latest 1.26.x at build time, which is `1.26.4` or newer).
4. **US037 ADR location: INLINE in this `architecture.md` as §9** (D-008). Not a new `docs/adr/` convention — REQ006 is the requirement that formalises the decision, so the rationale lives next to the rest of the REQ. The text appears in §9 below, ready for `read`-based verification by tech-lead.
5. **US036 TabSwitcher target: ≥80% stmts/branches/lines/functions, no `Home`/`End`** (D-009). The component does NOT implement `Home`/`End`; the AC already excludes them. FCT-* IDs pinned in §7.
6. **US032 `message.go` harness: httptest + Echo for `HandleMessage`, direct call for `sendError` + `sendToolResultError`** (D-010). Real `mcp.SessionManager` (no interface introduction) + real `mcp.ToolRegistry` with controllable tool registrations — matches existing handler-test convention.
7. **US038 Makefile consolidation (D-013, with US011 absorbed per D-014):** delete `startup.sh`/`shutdown.sh`; add `make dev-up/down/migrate/seed`. Native local Postgres on `:5432` (Q1); existing `e2e-*` family byte-identical (Q2); PID + log files at repo root mirror `startup.sh` byte-for-byte (Q3); BOTH `PG_CONN ?=` (e2e, port 15432) AND new `DEV_PG_CONN ?=` (dev, port 5432) env-overridable in the same change (absorbed from former US011, Option A union). Zero `DB_URL` in any new recipe. Closes tech-debt line 86 (PG_CONN hardcoded) and the original US011 ask. US038 SHOULD ship before or with US034.
8. **No new open questions left for the human.** OQ-1 → D-006. OQ-2 → D-007. OQ-3 → D-008. OQ-4 → carry-forward via §4.5 exemption mechanism. OQ-5 → D-010. US038 introduced under D-013 with Q1–Q4 already answered by the user; D-014 merges former US011 into US038 (Option A union — both `?=` overrides preserved). Nothing further to surface unless the human disagrees with a specific decision above.
...
- **Cluster 1 — `internal/repo/` error-branch backfill (BE-test only).** US025 (`task_repo.go`), US026 (`user_story_repo.go`), US027 (`audit_repo.go`).
- **Cluster 2 — `internal/handler/*_tools.go` error-mapping backfill (BE-test only).** US028 (`project_tools.go`), US029 (`document_tools.go`), US030 (`task_tools.go`), US031 (`user_story_tools.go`), US032 (`message.go`).
- **Cluster 3 — `internal/mcp/server.go` ToolRegistry + Session message methods (BE-test only).** US033.
- **Cluster 4 — Env-var harmonisation (BE-prod).** US034.
- **Cluster 5 — Housekeeping + ADR.** US035 (Go toolchain bump), US036 (FE TabSwitcher coverage), US037 (ADR — MCP-only-writes), US038 (Makefile consolidation, absorbs former US011 — retires `startup.sh`/`shutdown.sh` and flips `PG_CONN := → ?=`).
...
- **No production code changes in cluster-1/2 stories.** US025–US032 are tests-only. If a backfill test reveals a real bug (e.g. a missing `rows.Err()` check), the dev raises `ARCHITECTURE_GAP_FOUND` and the orchestrator routes back to system-architect for a new follow-up story — devs do NOT silently fix in this scope.
- **No retroactive REQ001-005 feature changes.** REQ001–005 shipped; this is debt cleanup.
- **No e2e additions.** Unit/integration backfill on BE, component-test backfill on FE. No new `.robot` files. (US038 and US035 incidentally exercise `make e2e-up/run/down`, but they do not add new e2e tests.)
...
|---|---|---|---|
| `services/agent-board/cmd/api-server` | modified (US034 env var, US035 toolchain) | Read-only REST surface (4 GETs on `/api/v1/...`) | Postgres only |
| `services/agent-board/cmd/mcp-server` | modified (US034 env var, US035 toolchain) | MCP write surface — SSE + `/message` JSON-RPC + tool registry | Postgres only |
| `services/agent-board/internal/{repo,handler,mcp,domain}` | test files modified (US025..US033); production code untouched | Domain + persistence + MCP plumbing | — |
| `services/agent-board/internal/config` | **NEW package** for US034 (one file, `dburl.go`) | Resolves which env var supplies `DATABASE_URL`; emits startup log | — |
| `web/components/ProjectDetail/TabSwitcher.tsx` | test file modified (US036); production code untouched | WAI-ARIA Tabs pattern for project detail page | — |

---

## 3. File-level touch map (US025..US038)
...
### US025 — `task_repo.go` error-branch tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/repo/task_repo_test.go` | edit | Add the 12 test functions named verbatim in US025 AC. Follow §4 pattern. Test-only. |
| `services/agent-board/internal/repo/task_repo.go` | **no change** | Byte-for-byte unchanged. |

### US026 — `user_story_repo.go` error-branch tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/repo/user_story_repo_test.go` | edit | Add 12 test functions per US026 AC. Same shape as US025. |
| `services/agent-board/internal/repo/user_story_repo.go` | **no change** | Byte-for-byte unchanged. |

### US027 — `audit_repo.go` error-branch tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/repo/audit_repo_test.go` | edit | Add 6 test functions per US027 AC (3 error branches × 2 public callers). |
| `services/agent-board/internal/repo/audit_repo.go` | **no change** | Byte-for-byte unchanged. |

### US028 — `project_tools.go` error-mapping tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/handler/project_tools_test.go` | edit | Add 18 test functions per US028 AC. Includes `TestRegisterProjectTools_RegistersAllFiveTools` (covers 0% on `RegisterProjectTools`). Use mock `repo.ProjectRepository` per §4.3. |
| `services/agent-board/internal/handler/project_tools.go` | **no change** | Byte-for-byte unchanged. |

### US029 — `document_tools.go` error-mapping tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/handler/document_tools_test.go` | edit | Add 20 test functions per US029 AC. Existing `MockDocumentRepo` already at top of file — reuse it. Includes `TestRegisterDocumentTools_RegistersAllFiveTools`. |
| `services/agent-board/internal/handler/document_tools.go` | **no change** | Byte-for-byte unchanged. |

### US030 — `task_tools.go` error-mapping tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/handler/task_tools_test.go` | edit | Add 25 test functions per US030 AC. `UpdateTaskTool` carries the bulk (5 distinct status-change branches + no-status-change branch). |
| `services/agent-board/internal/handler/task_tools.go` | **no change** | Byte-for-byte unchanged. |

### US031 — `user_story_tools.go` error-mapping tests

| File | New / Edit | Change |
|---|---|---|
| `services/agent-board/internal/handler/user_story_tools_test.go` | edit | Add 27 test functions per US031 AC. Note the **passthrough** error semantics (no `fmt.Errorf` wrap on most paths — verify via `errors.Is(returnedErr, mockErr)`). |
| `services/agent-board/internal/handler/user_story_tools.go` | **no change** | Byte-for-byte unchanged. |

### US032 — `message.go` error-routing tests

| File | New / Edit | Change |
|---|---|---|
| **`services/agent-board/internal/handler/message_test.go`** | **NEW file** | Create from scratch. Add 13 test functions per US032 AC, following §8 harness. Use real `mcp.SessionManager` + real `mcp.ToolRegistry` with controllable tool registrations. |
| `services/agent-board/internal/handler/message.go` | **no change** | Byte-for-byte unchanged. |

### US033 — `internal/mcp/server.go` ToolRegistry + session messaging tests

| File | New / Edit | Change |
|---|---|---|
| **`services/agent-board/internal/mcp/server_test.go`** | **NEW file** | Create from scratch. Holds the 15 test functions per US033 AC: `TestNewToolRegistry_*`, `TestToolRegistry_*`, `TestSession_*`, `TestSessionManager_RemoveSession_*`. Tester MAY split into `tool_registry_test.go` + `session_test.go` additions; the existing `session_test.go` is tiny and may be merged into. Test names are authoritative regardless of placement. |
...
### US034 — Standardise on `DATABASE_URL` (reject `DB_URL`)
...
| `docker-compose.yml` | edit | Change `mcp-server.environment.DB_URL` (line 48) → `DATABASE_URL` (same URL value). `api-server` already sets `DATABASE_URL`. Drop the pre-existing-inconsistency comment (lines 46–47) and replace with a one-line `# Standardised on DATABASE_URL per REQ006/US034 (D-006). DB_URL is rejected at startup.` Both services now set `DATABASE_URL` and ONLY `DATABASE_URL`. |
| `services/agent-board/cmd/api-server/main_test.go` | edit OR new | If startup-log integration test is added (US034 AC scenario "Scenario: startup log line is integration-tested"), place here. Tester's call between subprocess `os/exec` vs `log.SetOutput` capture; either acceptable. Asserts the single happy-path line `"db config: using DATABASE_URL"`. |
...
| `tests/e2e/README.md` | edit | If it references `DB_URL`, update to: "`DATABASE_URL` only; `DB_URL` is rejected at startup as of REQ006/US034". (Tech-lead to confirm by grep during planning.) |
| `docs/tech_debt.md` | edit | Strike-through line 97 with `→ fixed in REQ006/US034 (standardised on DATABASE_URL; DB_URL rejected at startup)`. |

**Note (US038 sequencing):** `startup.sh` and `shutdown.sh` at repo root are NOT in US034's touch map — they are **deleted by US038**, not edited by US034. The legacy `startup.sh` references `DB_URL` (which is exactly what motivated US038), but US034's be-dev MUST NOT touch those scripts: by the time US034 lands they may already be gone (US038 sequencing — see §11 D-013 and §13.1 R-6). If US034 lands first, the existing `startup.sh` workflow breaks at the next invocation with the hard-fail rename-instruction error from §5.4; that breakage is the trigger to merge US038 next. Tech-lead's call whether to pair both PRs into a single merge or sequence US038 → US034 in two PRs.

### US035 — Go toolchain bump
...
| `docs/tech_debt.md` | edit | Strike-through line 28 with `→ fixed in REQ006/US035`. |

### US036 — `TabSwitcher.tsx` coverage backfill
...
| `docs/tech_debt.md` | edit | Strike-through line 75 with `→ fixed in REQ006/US036`. |

### US037 — ADR — MCP-only-writes

| File | New / Edit | Change |
|---|---|---|
| `docs/requirements/REQ006_tech_debt_backfill_sprint/architecture.md` (this file) | edit | The ADR text is in **§9 below**, authored by system-architect in this revision. Tech-lead's task for US037 reduces to "verify §9 satisfies every AC scenario in `US037_*.md`." No code changes. |
| `docs/tech_debt.md` | edit | Strike-through line 98 with `→ won't-fix per REQ006/US037 ADR-001 (MCP-only-writes is permanent)`. |

### US038 — Consolidate dev workflow into the Makefile; retire `startup.sh`/`shutdown.sh`; flip existing `PG_CONN := → ?=` (absorbs former US011 per D-014)
...
| `docs/tech_debt.md` | edit | Strike-through **line 86** with `→ fixed in REQ006/US038 (absorbed from former US011 for line 86; PG_CONN now `?=` and env-overridable)`. **Additionally** if any line references `startup.sh` / `shutdown.sh` directly, strike-through with `→ fixed in REQ006/US038`. |

**Note (variable namespace — both `?=` overrides preserved per D-014, Option A union):** `PG_CONN ?=` (e2e family, port 15432, absorbed from former US011) and `DEV_PG_CONN ?=` (dev family, port 5432, new in US038) are deliberately separate variables on separate Makefile lines serving separate workflows. They use the same `?=` idiom but MUST NOT be collapsed into one variable. Option B (drop the e2e override and use only `DEV_PG_CONN`) was rejected at D-014 because it would re-open tech-debt line 86 (the original US011 ask).

**Note (sequencing with US034):** US038 SHOULD ship before or in the same merge as US034 — see §11 D-013 consequences and §13.1 R-6. No hard `Blocked by` link; tech-lead's call whether to pair the PRs.

**Note (no production-Go-code change):** US038 touches the Makefile + repo-root scripts + docs + agent definitions. Zero `services/agent-board/` Go source files are edited. The "no production code change for cluster-1/2/3 tests-only" rule remains accurate; US038 is a separate cluster-5 housekeeping touch, in the same shape as US035 (toolchain + Dockerfile).

**Note (no regression test for the `?=` flip):** The merged US038 AC explicitly excludes the standalone `scripts/review/test/test_makefile_pg_conn.sh` regression test that the original US011 had proposed. The `?=` change is exercised implicitly by `make e2e-up` continuing to work; a dedicated assertion script is out of scope.

---

## 4. Cluster 1 + 2 + 3 test pattern (the load-bearing section)

This section defines the **single shared test pattern** that US025..US033 follow. It is written ONCE here so devs and tester do not re-derive it per story. The pattern is a direct descendant of REQ005/US029's already-proven approach for `document_repo` / `project_repo`.

### 4.1 Reference: REQ005/US029

The canonical example. Re-read `services/agent-board/internal/repo/project_repo_test.go` and `services/agent-board/internal/repo/document_repo_test.go` for the exact shape: `sqlmock.New()` → `mock.ExpectQuery / ExpectExec / ExpectBegin / ExpectCommit` configured per branch → invoke repo method → assert `(returnValue, err)` pair → assert `mock.ExpectationsWereMet()`. Cluster 1 (US025–US027) mirrors this pattern across the remaining repos. Cluster 2 (US028–US032) lifts the same idea up one layer — replace `sqlmock` with a hand-written mock `repo.<Entity>Repository`. Cluster 3 (US033) drops to bare struct testing because the SUT is in-memory plumbing.
...
The `_RowsErr` / `_ScanError` shapes are sufficient to lift coverage past 95% on every cluster-1 file given REQ005/US029's empirical results on the analogous files.
...
`testify/mock` is acceptable as an alternative IF tester / dev prefers it (US028, US030 notes both allow). Hand-written wins on readability for a 5-method interface; `testify/mock` wins when many tests share setup. Pick once per file, do not mix.
...
### 4.4 Cluster 3 — bare-struct unit tests (US033 only)
...
|---|---|---|
| 1 | US025 | `cd services/agent-board && go test ./internal/repo -cover -v -run TestTaskRepo` |
| 1 | US026 | `cd services/agent-board && go test ./internal/repo -cover -v -run TestUserStoryRepo` |
| 1 | US027 | `cd services/agent-board && go test ./internal/repo -cover -v -run TestAuditRepo` |
| 2 | US028 | `cd services/agent-board && go test ./internal/handler -cover -v -run "TestHandle(Create\|Get\|Update\|Delete\|List)Project\|TestRegisterProjectTools"` |
| 2 | US029 | `cd services/agent-board && go test ./internal/handler -cover -v -run "TestRegisterDocumentTools\|Test(Create\|Get\|Update\|Delete\|List)Document(s?)Tool"` |
| 2 | US030 | `cd services/agent-board && go test ./internal/handler -cover -v -run "TestRegisterTaskTools\|Test(Create\|Get\|Update\|Delete\|List)Task(s?)Tool"` |
| 2 | US031 | `cd services/agent-board && go test ./internal/handler -cover -v -run "TestRegisterUserStoryTools\|Test(Create\|Get\|Update\|Delete\|List)UserStor(y\|ies)Tool"` |
| 2 | US032 | `cd services/agent-board && go test ./internal/handler -cover -v -run "TestHandleMessage\|TestSendError\|TestSendToolResultError"` |
| 3 | US033 | `cd services/agent-board && go test ./internal/mcp -cover -race -v` |

Every story also runs the broader `cd services/agent-board && go test ./... && golangci-lint run ./...` before review.

---

## 5. US034 — Env-var harmonisation contract
...
A small new package `services/agent-board/internal/config` with one file `dburl.go`. The helper remains (despite the simpler semantics) so US034's AC ≥95% per-package coverage requirement has a clean unit-test surface — without it, the four env-state cases would need to be exercised via subprocess-spawning `main_test.go` tests.
...
- US034's AC requires unit tests for the helper (≥95% per-package coverage). A package gives a single test surface (`dburl_test.go`) rather than two duplicate test files inside `cmd/api-server/` + `cmd/mcp-server/`.
...
- The precedent of REQ005 D-008 (duplicate 9-line context glue) does not apply: that decision was about avoiding a new package for code whose only test surface was the cmd-level test. US034 IS the test surface.

### 5.3 `main.go` call sites (post-US034)
...
| Only `DB_URL` set | Helper returns error; caller `log.Fatal`s | `"DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US034)"` |
| Both set | Helper returns error; caller `log.Fatal`s | `"DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US034)"` |
...
2. `TestResolveDBURL_OnlyDBURLSet_RejectsWithRenameError` — `DB_URL` set, `DATABASE_URL` unset. Assert `(url=="", err.Error() == "DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US034)")`.
3. `TestResolveDBURL_BothSet_RejectsWithDisambiguateError` — both set. Assert `(url=="", err.Error() == "DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US034)")`.
...
**Note vs. the existing US034 story AC:** the previously-listed `BothSet_PreferredWins` test case disappears because both-set is now an error, not a precedence resolution. The four cases above replace it. Reconciliation flag: see §5.8.

Package coverage ≥95% (trivial — four cases cover every line of a four-branch switch).

### 5.7 Integration test for startup log line (per US034 AC)
...
(b) is faster + more deterministic; (a) is more end-to-end. Both satisfy US034 AC.
...
### 5.8 US034 story-AC reconciliation flag (route to po-ba)

**The existing `US034_harmonise_db_url_env_var.md` story has AC that is now stale and must be revised by po-ba before Phase 2 picks it up.** Specifically:
...
System-architect does NOT edit the story file. The orchestrator must route to po-ba next (story revision) before Phase 2 spawns tester / tech-lead on US034. The other twelve stories (US025..US033, US035..US037) and the merged US038 (which absorbs former US011 per D-014) are unaffected by this revision and can proceed independently.

### 5.9 Migration impact (operator-facing — be explicit)

The post-REQ005 `docker-compose.yml` (D-009) uses `DATABASE_URL` for `api-server` and `DB_URL` for `mcp-server`. The mcp-server line is renamed in the same commit as the US034 code change. Internal infra: zero residual breakage.
...
REQ005 architecture (rev 4, still `pending_approval`) carries OQ-7 about env-var harmonisation that was deferred. US034 closes it: single-var contract + helper shape + error wording + log-line wording locked here. REQ005 architecture can reference D-006 / §5 when it lands.

---

## 6. US035 — Go toolchain bump
...
Additional dep-tree findings exist but are NOT called by this code (govulncheck classifies them as "vulnerabilities in modules you require but your code doesn't appear to call these") — these are noise for US035's purposes.
...
- 1.27 ships in mid-2026 per the Go release cadence (Feb + Aug); at REQ006 authoring it may not be released yet. US035 AC requires "lowest released minor that fixes" — 1.26.4 wins.
- 1.27 may introduce new vulnerabilities or new dep-tree compat issues not yet tested. 1.26.4 is the conservative pick.
- If 1.27 is released before US035 lands AND the human prefers it, that is a one-line override at re-approval; D-007 does not preclude.
...
- `cd services/agent-board && go test ./...` runs against the dev's local toolchain. Per US035 AC, dev installs `go1.26.4` locally (`go install golang.org/dl/go1.26.4@latest && go1.26.4 download`) OR upgrades the system toolchain.
...
After US035 lands, `cd services/agent-board && govulncheck ./...` MUST exit clean (zero `Your code is affected by N vulnerabilities` lines). Tester adds this as an explicit assertion in the test report. If a new finding emerges from the bump (transitive dep that flares on 1.26.4 stdlib), that becomes a NEW follow-up story — do not silently fix in US035.

---

## 7. US036 — TabSwitcher FE story scope
...
The tester will produce `US036_fe_unit_tests.md` containing the following FCT-* IDs in order. Each maps to a scenario in `US036_*.md` AC:

| FCT ID | Scenario | Mapping to AC scenario |
|---|---|---|
| FCT-US036-001 | Clicking non-active tab fires `onTabChange` with clicked id; component does not internally mutate `activeTab` | "Scenario: clicking a non-active tab fires `onTabChange` with the clicked tab id" |
| FCT-US036-002 | `ArrowRight` moves focus forward and fires `onTabChange` | "Scenario: `ArrowRight` moves focus and fires `onTabChange` to the next tab" |
| FCT-US036-003 | `ArrowRight` from last tab wraps to first | "Scenario: `ArrowRight` from the last tab wraps to the first" |
| FCT-US036-004 | `ArrowLeft` moves focus backward and fires `onTabChange` | "Scenario: `ArrowLeft` moves focus and fires `onTabChange` to the previous tab" |
| FCT-US036-005 | `ArrowLeft` from first tab wraps to last | "Scenario: `ArrowLeft` from the first tab wraps to the last" |
| FCT-US036-006 | `Enter` activates focused tab; calls `preventDefault` | "Scenario: `Enter` activates the focused tab" |
| FCT-US036-007 | `Space` activates focused tab; calls `preventDefault` | "Scenario: `Space` activates the focused tab" |
| FCT-US036-008 | `aria-selected` per tab reflects `activeTab` prop | "Scenario: `aria-selected` reflects the active tab" |
| FCT-US036-009 | Roving `tabIndex` per tab reflects `activeTab` prop | "Scenario: roving `tabIndex` reflects the active tab" |
| FCT-US036-010 | Prop-driven `activeTab` change re-renders with new active tab AND does NOT fire `onTabChange` | "Scenario: prop-driven `activeTab` override re-renders with the new active tab" |
| FCT-US036-011 | Tablist semantics (`role="tablist"`, `aria-label`, `aria-controls`, `id`) | "Scenario: tablist semantics are present" |
| FCT-US036-012 | Unrelated keys do NOT fire `onTabChange` AND do NOT call `preventDefault` | "Scenario: unrelated keys do not fire `onTabChange`" |
...
## 8. US032 — `message.go` test harness shape (resolves OQ-5)
...
### 8.5 The two `json.Marshal` fallbacks (US032 AC note)
...
> This is the inline ADR text for US037. The system-architect authored it during Phase 1 per US037's direction; tech-lead's role is verification, po-ba's role is sign-off. **ADR location decision (D-008): inline here, NOT a separate `docs/adr/` document.** Rationale: REQ006 is the requirement that formalises the decision; the rationale lives next to the rest of the REQ. The downstream verification cost (read this section, check it matches the US037 AC scenarios) is identical either way; the discoverability cost is lower (one fewer place to look). If a future REQ formalises a `docs/adr/` convention with more than one ADR, US037's content gets lifted into `docs/adr/0001-mcp-only-writes.md` as part of that future REQ.
...
- `e2e` test fixtures cannot rely on `POST /api/v1/projects` for setup — they must go through MCP tools (this is what REQ005/US032 implemented via the `mcp_keywords.resource` Robot keywords, and what `make e2e-up` enables by including mcp-server in compose per REQ005 D-015).
- Any tech-debt item that proposes "add REST writes" is now closed as `won't-fix` with a pointer to this ADR (US037 AC requires the strike-through in `docs/tech_debt.md` line 98).
...
- REQ005/US032 (e2e suite-setup rewrite to go through MCP — historical evidence the team has already lived with this ADR's consequences).

---

## 10. Skill / hook usage

### 10.1 BE stories (US025..US035)

- **TDG (Test Doctor / TDD-guide skill).** Mandatory per project convention. Devs write failing test first, prove it fails for the right reason, then implement (US034, US011, US035 only — US025..US033 / US036 are tests-only, so "TDG" reduces to "write the test, run it, see it pass against the existing untouched production code; if it passes without changes that means it was already covered or you wrote the wrong assertion — re-check").
- **Live e2e + 3-clean-run flake check** (REQ005/US032 mandate D-014). Any story that touches `services/<>/` production code (US034, US035) requires a `make e2e-up && make e2e-seed && make e2e-run && make e2e-down` clean run three times in a row before code review can pass. US038 (Makefile only, absorbing the former US011 `PG_CONN ?=` flip) requires the same — it changes `make` behaviour and the regression bar is "the e2e pipeline still works."
- **`govulncheck` clean** (US035 specifically). The bumped toolchain MUST produce zero findings reachable from the project's own code. Tester captures `govulncheck ./...` output in the US035 test report.

### 10.2 FE stories (US036)
...
- **React-doctor (skill `.claude/skills/react-doctor/`).** D-014 from REQ005 makes react-doctor mandatory for FE code touches. US036 does NOT touch production code, so react-doctor's rule-set applies only as a sanity sweep on the new test file (no `dangerouslySetInnerHTML` in test code, no large reducer cascades in test code — neither is a realistic risk).
- **Live e2e + 3-clean-run.** US036 is component-level only; it does not require an e2e run. (If it did, it would also need to hit the 3-clean-run bar.)

### 10.3 Meta / ADR story (US037)

- **No TDG, no live e2e, no react-doctor.** US037 is documentation-only; the deliverable is this §9.
- **Verification ≠ tests.** Tech-lead's verification task reads §9 and checks each AC scenario in `US037_*.md`. The "live e2e" for US037 is **a `grep`-based assertion** as part of the tech-lead review:
  ```
  ...
  ```
  Plus a manual read against the US037 AC. Tester writes US037's spec files (`US037_be_unit_tests.md`, `US037_fe_unit_tests.md`, `US037_e2e_tests.md`) as one-line disclaimers per US037 AC.

### 10.4 What counts as "live e2e" for cluster-1/2/3 (US025..US033)
...
Source: po-ba README. Implemented as US036.
...
- **Alternatives rejected:**
  ...
  - **Duplicate the helper into both `cmd/*/main.go` files (per REQ005 D-008 precedent).** Rejected: US034 AC requires unit tests for the helper (≥95% per-package coverage), which is cleaner with a package than two duplicate test files; the four-branch behaviour is more substantive than REQ005's 9-line lifecycle glue.
- **Consequences:**
  ...
  - **Forces a US034 story-AC reconciliation by po-ba** before Phase 2 spawns for US034 — see §5.8. The three pre-existing AC scenarios that describe dual-acceptance / precedence / `BothSet_PreferredWins` are now stale and must be replaced.
...
- **Consequences:**
  - Tech-lead's US037 verification is a grep+read on §9 of this file.
  - Future ADRs may live anywhere; this decision is REQ006-local.

### D-009 — US036 coverage target: 80% across stmts / branches / lines / functions
- **Context:** US036 AC specifies 80%; po-ba intake confirmed.
...
### D-010 — US032 message.go harness: httptest + Echo for `HandleMessage`, direct call for helpers
...
- **Consequences:** US032 AC's 13 test functions map cleanly to httptest invocations plus 2-4 indirect helper coverage assertions.

### D-011 — Cluster-1/2 `≥95% modulo enumerated unreachable lines` carries forward from REQ005/US029
- **Context:** OQ-4 (some lines genuinely unreachable via sqlmock).
- **Decision:** Exemption mechanism in §4.5: tester names each unreachable line in the test report with a one-line justification. Threshold becomes `≥95% modulo enumerated unreachable lines`. Same as REQ005/US029's precedent.
...
- **Context:** US034 needs a unit-test surface for the env-var helper.
- **Decision:** `services/agent-board/internal/config/dburl.go` + matching `_test.go` is the ONLY new production-code file across all of REQ006. Every other production file is either edited (US034 cmd/*, US035 go.mod + Dockerfile, US038 Makefile — which now also carries the absorbed US011 `PG_CONN := → ?=` flip per D-014) or untouched (everything else). US038's deletions of `startup.sh`/`shutdown.sh` and its Makefile edits do not introduce any new production-Go-code files.
- **Consequences:** Tech-lead's "no production code change" assertion for US025–US033 + US036 remains accurate; US034 carves out ONE new file for the helper, justified by the test surface.

### D-013 — Retire `startup.sh`/`shutdown.sh`; consolidate dev + docker workflows in the Makefile (US038)
- **Context:** During the Phase 1 HARD STOP for REQ006 architecture rev 2, the human identified that `startup.sh` at repo root (line 16) still passes `DB_URL=...` to `mcp-server`, which would hard-fail once US034's single-var contract (D-006) lands. Rather than spot-patching env-var names in the legacy script, the human directed the team to **retire `startup.sh`/`shutdown.sh` entirely** and make the `Makefile` the single entry point for both local-dev and docker e2e workflows. **Verbatim user direction (recorded for future readers):** *"just ignore startup/shutdown env name, delete it, and enhance make file to work for both docker and dev local for all action like start/stop/migration_db/seed data in us015"*.
- **Decision:** A new story **US038** (cluster 5, housekeeping) was authored by po-ba to carry this work. Architecture impact:
  ...
  - Zero `DB_URL` in any new recipe — validates US034 alignment.
- **Sub-decisions locked from po-ba intake (Q1–Q4):**
  - **Q1 — Postgres source for `dev-*`:** native install only. Operator installs `postgres` on `localhost:5432`; `make dev-up` does NOT start a DB container. Recommended macOS path documented in US038 `Notes for the team`; cross-platform install docs are explicitly out of scope.
  ...
  - **Q4 — Relationship with US011:** US011 (`PG_CONN ?=` for the e2e family) stays an independent story. US038 introduces a separate `DEV_PG_CONN ?=` for the dev family. Same `?=` idiom, different variables, different default ports, different workflows. No hard `Blocked by` link in either direction.
    - **Q4 subsequently superseded by D-014 (merge during HARD STOP rev 4):** US011 was absorbed into US038 as part of the Phase 1 HARD STOP rev-4 round. Both `?=` overrides (`PG_CONN ?=` AND `DEV_PG_CONN ?=`) now live together in US038 (Option A union). The "independent" framing was historically correct at intake; the merge during rev 4 made it redundant. See D-014 for full context.
- **Alternatives rejected:**
  - **(a) Spot-patch the env-var name in `startup.sh` as part of US034.** Rejected by the human at the Phase 1 HARD STOP — leaves a divergent local-dev script with no consolidation benefit; the legacy script remains tech-debt indefinitely.
  ...
  - **(d) Merge US011 into US038.** Rejected — loses the per-story granularity REQ006 maintains elsewhere; the two changes can land in any order and serve distinct workflows.
...
- **Consequences:**
  - **Sequencing with US034:** US038 SHOULD ship before or in the same merge as US034. **Rationale:** US034's hard-fail on `DB_URL` would break the existing `startup.sh` workflow until US038 deletes it. No hard `Blocked by` — tech-lead may pair both PRs in one merge, or sequence US038 → US034 in two PRs.
  - **§3 US034 row** carries an explicit note that `startup.sh`/`shutdown.sh` are NOT in US034's lane — they are deleted by US038. US034's be-dev MUST NOT touch them.
  - **README story count** moves from 14 → 15. Cluster 5 grows from 4 stories to 5. **(Subsequently revised by D-014: US011 absorbed into US038 → story count returns to 14, cluster 5 returns to 4.)**
  - **Sweep impact:** US038 includes documentation + agent-definition references to `startup.sh`/`shutdown.sh` — `README.md`, `tests/e2e/README.md`, `services/agent-board/README.md` (if exists), `.claude/agents/*.md`, `CLAUDE.md`. Plus a `python3 scripts/sync-gemini.py` run after agent-file edits per project rule.
  - **No production-Go-code change.** US038 is Makefile + repo-root scripts + docs + agent-defs only. The D-012 "only `internal/config` is the new production file" claim still holds.
  - **No new architecture OQ raised.** Q1–Q4 were answered by the human at po-ba intake before this revision was written.

### D-014 — US011 absorbed into US038 (merge during Phase 1 HARD STOP rev 4)
- **Context:** User identified during the Phase 1 HARD STOP rev-4 round that US011 (1-line `PG_CONN := → ?=` flip) was redundant with US038 (Makefile consolidation). Both are Makefile env-overridability hygiene; same file, same `?=` pattern. A standalone 1-line story for a Makefile flip that lives next door to four new `dev-*` targets and a new `DEV_PG_CONN ?=` variable is conceptual overlap with negligible discrete value.
- **Decision:** Merge US011 INTO US038. Delete the `US011_makefile_pg_conn_overridable.md` story file. US038 AC now covers BOTH `?=` variables. **Option A (union) selected** — BOTH `PG_CONN ?=` (e2e family, port 15432, default URL byte-identical to the pre-flip value) AND `DEV_PG_CONN ?=` (dev family, port 5432, new in US038) are preserved in the merged Makefile. Option B (drop the e2e override and use only `DEV_PG_CONN`) was rejected because it would re-open tech-debt line 86 (the original US011 ask).
- **Alternatives rejected:**
  ...
  - **(b) Rename US038 to "Makefile consolidation + env-overridability" without merging US011.** Rejected: the current US038 title already covers the consolidation work that the merged scope still fits under; renaming is cosmetic and does not address the underlying duplication.
...
- **Consequences:**
  - **Story count drops from 15 → 14.** Cluster 5 drops from 5 → 4 stories (now `US035, US036, US037, US038`).
  - **Architecture §3 US011 row removed.** §3 US038 row expanded to include the existing-line `PG_CONN := → ?=` flip and the `docs/tech_debt.md` line 86 strike-through obligation (previously owned by US011).
  ...
  - **The `scripts/review/test/test_makefile_pg_conn.sh` regression test** that the original US011 had optionally proposed is **explicitly out of scope** per the merged US038 AC. The `?=` change is exercised implicitly by `make e2e-up` continuing to work.
  - **No re-routing required for tester or tech-lead.** The merge happened during Phase 1 HARD STOP rev 4, before Phase 2 spawns. Tester will produce one set of spec files for US038 covering both the `dev-*` targets and the `PG_CONN ?=` flip; tech-lead will produce US038 task(s) for the merged scope.
  - **README (po-ba's) and the story file** are already updated by po-ba (story count 15 → 14, cluster 5 row 5 → 4, new README D-007 entry documenting the merge, OQ-6 updated, `US011_makefile_pg_conn_overridable.md` deleted, US038 story file expanded with the absorbed scope). This architecture revision (rev 4) reconciles the architecture-side documents.
...
- `PG_CONN` (Makefile, e2e family, port 15432) and `DEV_PG_CONN` (Makefile, dev family, port 5432) are both env-overridable via `?=` (US038, absorbing former US011 per D-014) — unchanged default values.

### 12.2 Logging keys

- US034 introduces the `db config: using ...` log line at startup, before the DB ping. Stay consistent with the existing `log.Printf` calls (no log levels — Go stdlib's `log` package has none).
...
Unchanged. REQ005/US032 added container logs via `make e2e-logs`; REQ006 does not extend this.
...
1. **R-1: Bumping Go toolchain to 1.26.4 may surface new transitive dep issues.** Mitigation: US035 AC requires `cd services/agent-board && govulncheck ./... && go test ./... && go build ./...` clean post-bump. If a new finding flares, raise a follow-up story; do not silently widen US035's scope.
...
4. **R-4: hard-fail on `DB_URL` will break any external deployment (Helm chart, custom compose override, CI script, hand-set shell env) that has been passing `DB_URL` to mcp-server.** Mitigation: this is intentional and accepted per D-006's revised decision — the rename-instruction error message is operator-actionable, the migration cost is one env-var rename per affected deployment, and the upside is no silent-misconfig risk during the migration. Surfaced in §5.9 explicitly so the orchestrator can flag it in the US034 test report and so the human can re-evaluate at re-approval if the appetite for operator-impact changes. Internal infra: zero residual breakage — `docker-compose.yml` is updated in the same commit.
5. **R-5: The doc-comment lie on `ListTools` ("lexicographic order") going unfixed may confuse future contributors.** Mitigation: US033 AC explicitly flags it in the test report under OQ-4; tech-debt entry added; not silently fixed in US033's tests-only scope.
6. **R-6: If US034 lands before US038, the existing `startup.sh` workflow breaks at the next invocation** with the hard-fail rename-instruction error from §5.4 (because `startup.sh:16` passes `DB_URL` to mcp-server). Mitigation: D-013 explicitly directs sequencing — US038 SHOULD ship before or in the same merge as US034. Tech-lead's call whether to pair both PRs into one merge or sequence US038 → US034 in two PRs. The breakage is loud (operator-actionable startup error), not silent; the recovery is "run `make dev-up` instead of `./startup.sh`" once US038 has landed. Acceptable.
...
- US038 sub-decisions Q1–Q4 → D-013 (locked at po-ba intake; no architecture-side OQs raised).
- US011-vs-US038 merge (po-ba README OQ-6 in its local sequence) → D-014 (US011 absorbed into US038, Option A union — both `?=` overrides preserved).
...
Initial draft. Resolves OQ-1..OQ-5 from po-ba README via D-006..D-012. Inline ADR-001 in §9 (US037 deliverable). File-level touch map covers US025..US037. Cluster-1/2/3 test pattern locked in §4 once for all 9 BE-test stories. `Approval: pending_approval` set.

### Revision 2 — 2026-06-03 — driver: human feedback pass 1
US034 env-var contract revised end-to-end. The human rejected the precedence-rule approach from Revision 1 during the Phase 1 HARD STOP because of silent-misconfiguration risk during the migration. The revised contract is **single-var only**: `DATABASE_URL` is the SOLE accepted env var; `DB_URL` is REJECTED at startup with a fatal, operator-actionable error.
...
- **§3 US034 touch map** — heading renamed ("Standardise on `DATABASE_URL` (reject `DB_URL`)"); per-row edit descriptions updated to reflect the hard-fail; mcp-server row now explicitly notes this is the meaningful runtime change with breaking-deployment implications; docker-compose row drops any "still accepting both" language; tests/e2e/README.md row updated to reflect rejection; tech_debt.md strike-through note revised.
- **§5** — full rewrite. New shape: decision statement + rationale (5.1), simplified helper API `ResolveDBURL() (string, error)` (5.2), call-site snippets (5.3), four locked error-message strings (5.4), docker-compose change (5.5), four-case unit-test plan (5.6), integration test + optional hard-fail regression test (5.7), **US034 story-AC reconciliation flag for po-ba (5.8)**, operator-facing migration impact (5.9), REQ005 OQ-7 closure note (5.10).
...
Sections deliberately untouched (per the revision brief — these are independent of the US034 contract): Clusters 1/2/3 (§4, §6, §8), US011/US035/US036/US037 (§6, §7, §9), FE TabSwitcher (§7), cross-cutting §12.2–12.6 other than §12.1, ADR-001 (§9), D-007..D-012.

**Routing note for orchestrator:** before Phase 2 spawns tester / tech-lead on US034, route to po-ba to revise `US034_harmonise_db_url_env_var.md` per §5.8. The other 13 stories (US025..US033, US011..US037) are unaffected and can proceed independently.

`Approval: pending_approval` re-set. Awaiting human re-approval at HARD STOP.

### Revision 3 — 2026-06-03 — driver: human feedback pass 2 — surfaced US038 scope from startup.sh DB_URL reference
At the Phase 1 HARD STOP for revision 2, the human surfaced that `startup.sh` at repo root still references `DB_URL` and directed the team to retire `startup.sh`/`shutdown.sh` entirely rather than spot-patch the env-var name. **Verbatim user direction:** *"just ignore startup/shutdown env name, delete it, and enhance make file to work for both docker and dev local for all action like start/stop/migration_db/seed data in us015"*. po-ba authored a new story `US038_consolidate_dev_and_docker_workflow_in_makefile.md` with locked sub-decisions Q1–Q4. This architecture revision reconciles to the new 15-story scope.

Sections touched (revision 3):
- **§0 reading guide** — story-count phrasing in the §3 / §11 section pointers updated (`US025..US037` → `US025..US038`; `D-006..D-012` → `D-006..D-013`).
- **§0.1 executive summary** — new bullet 7 for US038 summarising D-013 (delete `startup.sh`/`shutdown.sh`; add `dev-up`/`dev-down`/`dev-migrate`/`dev-seed`; Q1–Q4 sub-decisions; `DEV_PG_CONN ?=` distinct from US011's `PG_CONN ?=`; sequencing with US034). Previous bullet 7 (OQ summary) renumbered to bullet 8 and extended with the US038 OQ-closure note.
- **§1.1 in-scope** — "fourteen stories" → "fifteen stories"; Cluster 5 grown from 4 stories to 5 (US011, US035, US036, US037, US038).
- **§3 heading** — `US025..US037` → `US025..US038`.
- **§3 US034 row** — appended a non-table note that `startup.sh`/`shutdown.sh` are NOT in US034's lane (US038 deletes them). Tech-lead routing for the sequencing decision called out (pair PRs or sequence US038 → US034). No table row changes.
- **§3 US038 row (NEW)** — full touch map per po-ba US038 AC: explicit `DELETE` markers on `startup.sh` and `shutdown.sh`; four new Makefile targets; new `DEV_PG_CONN ?=` variable; existing `e2e-*` family byte-identical; sweep across `README.md`, `tests/e2e/README.md`, `services/agent-board/README.md` (conditional), `.claude/agents/*.md` (with `sync-gemini.py` mandate), `CLAUDE.md` (conditional), `docs/tech_debt.md` (conditional strike-through). Three explanatory notes: namespace coordination with US011, sequencing with US034, no-production-Go-code-change.
- **§11 D-012 consequences** — appended US038's Makefile edit to the list of edited production files; explicitly noted no new Go source files from US038.
- **§11 D-013 (NEW)** — full decision entry covering context (verbatim user direction at HARD STOP), decision (delete scripts + four Makefile targets + `DEV_PG_CONN ?=`), Q1–Q4 sub-decisions locked from po-ba intake, five rejected alternatives (spot-patch, switchable docker/native dev DB, rename `e2e-*` to `docker-*`, merge US011 into US038, process supervisor), and consequences (sequencing, README count bump, sweep impact, no new production-Go-code, no new OQ).
- **§13.1 R-6 (NEW)** — risk entry covering US034-lands-before-US038 sequencing breakage. Mitigation: D-013 sequencing direction; loud not silent breakage; recovery is "run `make dev-up` instead of `./startup.sh`".
- **§13.2 OQ summary** — added US038 Q1–Q4 closure bullet; D-013 added to the disagreement-callout list.

Sections deliberately untouched (per the revision brief):
- **§4** test pattern (clusters 1+2+3) — no impact from US038.
- **§5** US034 contract (decision text, helper API, four error messages, compose change) — untouched. Only the §3 US034 row added a sequencing note; §5's body is unchanged.
- **§6** US035 Go toolchain bump — no impact.
- **§7** US036 TabSwitcher — no impact.
- **§8** US032 message.go harness — no impact.
- **§9** ADR-001 — no impact.
- **§10** skill / hook usage — no impact (US038 follows the same Makefile-only convention as US011; no new hook rules).
- **§12** cross-cutting — no impact (US038 introduces `DEV_PG_CONN` as a Makefile variable, not an env var passed to services; the §12.1 "sole accepted env var" wording for the services already stands).
- **D-006 through D-012** in §11 — untouched (D-006 retains its env-var meaning; D-013 is the new entry for US038).

`Approval: pending_approval` re-set (unchanged from rev 2). Orchestrator should re-surface the executive summary to the human via `AskUserQuestion` for the third re-presentation. The 7-bullet form (rev-1 OQ-resolution shape + rev-2 single-var pivot + rev-3 US038 addition) is in §0.1.

**Routing note for orchestrator (carries forward from rev 2 + adds):** before Phase 2 spawns tester / tech-lead on US034, the rev-2 routing to po-ba for `US034_*.md` AC revision per §5.8 still stands. US038 is already authored by po-ba (`US038_consolidate_dev_and_docker_workflow_in_makefile.md`) and can proceed into Phase 2 without further story-side revision. The other 13 stories (US025..US033, US011..US037) remain unaffected and can proceed independently.

### Revision 4 — 2026-06-03 — driver: human feedback pass 3 — US011 absorbed into US038 (Option A union, both `?=` overrides preserved)
At the Phase 1 HARD STOP for revision 3, the human identified that US011 (1-line `PG_CONN := → ?=` flip) was redundant with US038 (Makefile consolidation). Both are Makefile env-overridability hygiene; same file, same `?=` pattern. po-ba merged US011 INTO US038: the `US011_makefile_pg_conn_overridable.md` story file is deleted; the US038 story file absorbs the `PG_CONN ?=` flip and the `docs/tech_debt.md` line 86 strike-through obligation. **Option A (union) selected** — BOTH `PG_CONN ?=` (e2e, port 15432) AND `DEV_PG_CONN ?=` (dev, port 5432) are preserved in the merged Makefile. Option B (drop the e2e override) was rejected because it would re-open tech-debt line 86. This architecture revision (rev 4) reconciles the architecture-side documents.

Sections touched (revision 4):
- **§0 reading guide** — `D-006..D-013` → `D-006..D-014`.
- **§0.1 executive summary** — bullet 7 (US038) rewritten to explicitly include the absorbed `PG_CONN ?=` flip from former US011 (Option A union); bullet 8 (OQ summary) extended with D-014 closure note.
- **§1.1 in-scope** — "fifteen stories" → "fourteen stories"; cluster 5 enumeration drops US011 (now reads `US035, US036, US037, US038`); US038 description updated to flag absorption of former US011 and the `PG_CONN ?=` flip.
- **§3 file-level touch map** — **US011 row deleted in full.** US038 row expanded with the existing-line `PG_CONN := → ?=` flip (default URL byte-identical) and the merged `docs/tech_debt.md` line 86 strike-through obligation. New note documents Option A union (both `?=` overrides preserved) and the explicit out-of-scope status of the standalone `test_makefile_pg_conn.sh` regression test.
- **§10.1 BE skill / hook usage** — US011 reference replaced with US038 (same `make e2e-*` regression bar still applies; now owned by the merged story).
- **§11 D-012 consequences** — edited-production-files enumeration drops the US011 Makefile reference (US038 already covers Makefile edits; the absorbed scope rolls into the same line).
...
- **§11 D-013 consequences** — README story-count line annotated with "Subsequently revised by D-014: US011 absorbed into US038 → story count returns to 14, cluster 5 returns to 4."
- **§11 D-014 (NEW)** — full decision entry: context (HARD STOP rev-4 user observation of redundancy), decision (merge US011 into US038, Option A union — both `?=` overrides preserved), three rejected alternatives (keep both, rename-only, Option B which would re-open line 86), consequences (story count 15 → 14, cluster 5 5 → 4, §3 US011 row removed and US038 row expanded, D-013 Q4 annotated, regression test out of scope, no re-routing required for downstream agents).
- **§12.1 cross-cutting config** — `PG_CONN` env-overridability line now references US038 (with absorbed-US011 callout via D-014) and mentions BOTH variables (`PG_CONN` and `DEV_PG_CONN`).
- **§13.1 R-3** — "15 stories" → "14 stories".
- **§13.2 OQ summary** — added US011-vs-US038 merge closure bullet pointing at D-014; disagreement-callout list extended to include D-014.

Sections deliberately untouched (per the revision brief):
- **§2** service topology — no impact.
- **§4** test pattern (clusters 1+2+3) — no impact.
- **§5** US034 contract (decision text, helper API, four error messages, compose change, §5.8 reconciliation flag) — no impact.
- **§6** US035 Go toolchain bump — no impact.
- **§7** US036 TabSwitcher — no impact.
- **§8** US032 message.go harness — no impact.
- **§9** ADR-001 — no impact.
- **§10.2 / §10.3 / §10.4** — no impact (US038 already exists under §10.1 via the same `make e2e-*` regression rule that previously named US011).
...
**No invalidation of prior-revision work.** Revision 4 is a targeted documentation reconciliation following the po-ba-side story merge. No spec files, task files, or code have been produced yet (still pre-Phase-2). The other 13 stories (US025..US033, US034, US035..US037) are unaffected by this revision; their content carries forward identically from revisions 1–3.

`Approval: pending_approval` re-set (unchanged from revs 2 and 3). Orchestrator should re-surface the executive summary to the human via `AskUserQuestion` for the fourth re-presentation. The 7-bullet form in §0.1 covers the cumulative state (rev-1 OQ-resolution + rev-2 single-var pivot + rev-3 US038 addition + rev-4 US011 absorption into US038).

**Routing note for orchestrator (carries forward from rev 3 + updates):** before Phase 2 spawns tester / tech-lead on US034, the rev-2 routing to po-ba for `US034_*.md` AC revision per §5.8 still stands. US038 is already authored by po-ba (`US038_consolidate_dev_and_docker_workflow_in_makefile.md`) with the absorbed US011 scope; it can proceed into Phase 2 without further story-side revision. The deleted `US011_makefile_pg_conn_overridable.md` is no longer routable. The other 12 stories (US025..US033, US035..US037) remain unaffected and can proceed independently.
