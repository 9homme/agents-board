# US020 — Harmonise `useProject`, `useProjectDocuments` on AbortController + signal-threaded `lib/api/`

**Story:** US020 — Harmonise `useProject`, `useProjectDocuments`, `useDocument` on AbortController + signal-threaded `lib/api/`
**Requirement:** REQ005
**Track:** FE
**Status:** completed
...
US024's two FE tasks `Blocked by:` this task. This is NOT a scaffold task in the orchestrator sense (no shared file like `web/package.json` or `lib/api/types.ts` is modified), but it is the **gate task** for US024 per architecture §11.4.

## Test contract

The dev must make these tests pass (from `US020_fe_unit_tests.md`, IDs assigned by tester — FCT-*):
...
- `fetchClient` already plumbs `signal` per §2 US020 row note — verify no change is needed; if you find a hole, raise `ARCHITECTURE_GAP_FOUND`.
...
- FCT-* passing: FCT-US020-001, -002, -003, -004, -006, -007, -009, -010.
...
- Unblocks: US024_fe_mermaid_diagram_ref_attach.md, US024_fe_use_document_reducer_and_documents_tab.md.

### Review pass 1 — 2026-06-02 — tech-lead (inline orchestrator review) — verdict: approved

- **NOTE on review modality:** Tech-lead subagent review aborted with `STALE_WORKTREE_BASE` (harness bug fixed by US023). Inline orchestrator review used as recovery.
- `cd web && npm run typecheck`: **clean** (rc=0).
- `cd web && npm test -- --watchAll=false`: **17 suites passed, 117 tests passed** (107 pre-existing + 10 new FCT-US020-*). Jest worker force-exit at end is the known MSW leak (deferred per D-001).
...
- AbortError handling verified per architecture §4.5 — FCT-US020-009 / -010 (dev's tests) assert that aborted requests do NOT surface as state-level error; tests passed.
