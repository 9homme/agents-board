...
## Scope
- **In:**
  - `web/pages/projects/[id].tsx` — CSR-only Pages Router page. Reads `id`, `tab` from `useRouter().query`. Treats missing/unrecognized `tab` as `'documents'`. Renders `<ProjectHeader>` (or its loading/404/error variants) + `<TabSwitcher>` + the active tab body. Active tab body is either `<UserStoriesTab/>` or a placeholder `<div data-testid="documents-tab-placeholder"/>` (US013 replaces the placeholder with `<DocumentsTab/>`). Hides the tab switcher entirely when the project fetch resolved to 404 or 500 — only the header-area "Project not found" / "Failed to load project" message + a "Back to dashboard" `<Link>` are visible in those states.
...
- **Out:**
  ...
  - Markdown / mermaid rendering — US014.
...
- The Documents tab placeholder slot is an opaque `<div data-testid="documents-tab-placeholder" role="tabpanel" id="tabpanel-documents" aria-labelledby="tab-documents" />` so that the WAI-ARIA wiring is already correct when US013 swaps its body in.
...
- `web/pages/projects/[id].test.tsx` (new) — FCT-US012-005, 007, 008, 009, 011, 014, 015
- `web/components/ProjectDetail/ProjectHeader.tsx` (new) — loading/notfound/error/loaded variants
- `web/components/ProjectDetail/ProjectHeader.test.tsx` (new) — FCT-US012-006 + supporting cases
- `web/components/ProjectDetail/TabSwitcher.tsx` (new) — WAI-ARIA tablist
- `web/components/ProjectDetail/TabSwitcher.test.tsx` (new) — FCT-US012-008, 009, 010
- `web/components/ProjectDetail/UserStoriesTab.tsx` (new) — verbatim placeholder copy
- `web/components/ProjectDetail/UserStoriesTab.test.tsx` (new) — FCT-US012-012, 013
...
- FCT-US012-005 through FCT-US012-015 covered (excluding FCT-US012-001 through FCT-US012-004 which belong to `us001_fe_project_card_link` task per test spec — those tests reference `ProjectCard.tsx`)

### Follow-up notes
- FCT-US012-001 through FCT-US012-004 are in `US012_fe_unit_tests.md` but belong to the `us001_fe_project_card_link` task (modifying `ProjectCard.tsx`). This task covers FCT-US012-005 through FCT-US012-015.
...
**Test contract coverage (FCT-US012-005 … FCT-US012-015) — verified across the dev's Jest suites:**

| FCT ID | File | Verdict |
|---|---|---|
| FCT-US012-005 (header renders name + description) | `web/pages/projects/[id].test.tsx:26-40` | PASS |
| FCT-US012-006 (empty description → "No description" placeholder) | `web/components/ProjectDetail/ProjectHeader.test.tsx:15-26` | PASS |
| FCT-US012-007 (loading skeleton visible during fetch) | `web/pages/projects/[id].test.tsx:42-60` (uses `delay('infinite')`) | PASS |
| FCT-US012-008 (two tabs, Documents default) | `web/components/ProjectDetail/TabSwitcher.test.tsx:6-21` and `web/pages/projects/[id].test.tsx:62-85` | PASS |
| FCT-US012-009 (click User Stories → URL update with shallow:true) | `web/components/ProjectDetail/TabSwitcher.test.tsx:23-30` and `web/pages/projects/[id].test.tsx:87-105` (asserts `router.replace` called with `{tab:'user-stories'}` + `{ shallow: true }`) | PASS |
| FCT-US012-010 (click Documents from User Stories re-activates) | `web/components/ProjectDetail/TabSwitcher.test.tsx:32-45` | PASS |
| FCT-US012-011 (`?tab=user-stories` activates User Stories on mount) | `web/pages/projects/[id].test.tsx:107-125` | PASS |
| FCT-US012-012 (UserStoriesTab verbatim text, em dash) | `web/components/ProjectDetail/UserStoriesTab.test.tsx:6-12` | PASS |
| FCT-US012-013 (UserStoriesTab no network calls) | `web/components/ProjectDetail/UserStoriesTab.test.tsx:14-23` (MSW unhandled-request mode catches any call) | PASS |
| FCT-US012-014 (404 → "Project not found" + tabs hidden + Back link) | `web/pages/projects/[id].test.tsx:127-141` | PASS |
| FCT-US012-015 (500 → "Failed to load project" + Back link) | `web/pages/projects/[id].test.tsx:143-153` | PASS |

FCT-US012-001..004 are correctly delegated to the sibling task `us001_fe_project_card_link` per `US012_fe_unit_tests.md`'s coverage matrix — no missing coverage.
...
- Documents tab placeholder slot has the exact WAI-ARIA wiring spec'd for US013 hand-off (`role="tabpanel"`, `id="tabpanel-documents"`, `aria-labelledby="tab-documents"`, `data-testid="documents-tab-placeholder"`; `web/pages/projects/[id].tsx:83-89`). PASS.
...
**Scope discipline:**
- All `In:` items present; all `Out:` items respected. `web/lib/api/client.ts` untouched (signal pass-through stays in US013 scope). `web/components/Dashboard/ProjectCard.tsx` untouched (sibling task's domain).
- Minor in-scope addition: `web/lib/api/types.ts` adds `DocumentListItem` / `DocumentsListResponse` / `Document` interfaces. These match the architecture's §"Frontend TypeScript interface mapping" verbatim and are pure type-only additions used by no production code in this task. Strictly the architecture assigns those types to US013, but pre-baking them as types (no runtime impact) is harmless and avoids a future merge collision on `types.ts`. The dev flagged this in `## Notes → Follow-up notes`. Accepted; the US013 FE task should treat these types as already-existing.
