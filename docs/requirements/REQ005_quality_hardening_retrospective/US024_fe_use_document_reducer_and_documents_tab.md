# US024 — `useDocument` → `useReducer` + `DocumentsTab` render-time selection

**Story:** US024 — Fix React-Doctor baseline regressions (top-3 state/effect + 1 security)
**Requirement:** REQ005
**Track:** FE
**Status:** completed
...
**Blocked by:** US020_fe_harmonise_hooks_on_abortcontroller.md
...
- `web/components/ProjectDetail/DocumentsTab.test.tsx` (one targeted assertion change per §2 US024 row — relax `router.replace`-on-auto-select to user-click only; NO weakening of behaviour-preserving assertions; tester re-specs this in `fe_unit_tests.md` before code lands)

Independent of `US024_fe_mermaid_diagram_ref_attach.md`. Both US024 tasks `Blocked by:` US020's FE task per §11.4 and are NOT blocked by each other.

## Test contract

The dev must make these tests pass (from `US024_fe_unit_tests.md`, IDs assigned by tester):
...
- FCT-* `DocumentsTab`: `router.replace` is called ONLY when the user clicks a sidebar item (via `handleSelectDoc`) — NOT on initial auto-select (this is the relaxed assertion per §2 US024 row + R8).
...
Per architecture §11 the seven rule IDs targeted across US024 are:
...
- AbortController contract from US020 preserved verbatim (race-safety unchanged).
...
**Files touched:**
- `web/hooks/useDocument.ts` — refactored from `useState×3` to `useReducer` per §11.2. State `{ data, isLoading, error }`, actions `FETCH_STARTED|FETCH_SUCCEEDED|FETCH_FAILED|ABORTED`. Public shape `{ data, isLoading, error, refetch }` unchanged. AbortController + `latestIdRef` race-safety preserved verbatim from US020.
- `web/hooks/useDocument.test.ts` — added FCT-US024-005 through FCT-US024-009 (reducer action assertions + public shape check).
...
- `web/components/ProjectDetail/DocumentsTab.test.tsx` — relaxed FCT-US016-003 per R8 (no more router.replace on mount assertion); added FCT-US024-010 through FCT-US024-012.

**Tests added:** 8 new tests (FCT-US024-005 through FCT-US024-012)
...
**FCT-US024-013 (react-doctor meta-assertion):** Verified — zero findings on `useDocument.ts` for rules #2–#4; zero findings on `DocumentsTab.tsx` for rules #5–#7. Score 100/100.
