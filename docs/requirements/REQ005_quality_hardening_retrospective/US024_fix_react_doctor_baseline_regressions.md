# US024 — Fix React-Doctor baseline regressions (top-3 state/effect + 1 security)
...
- Any change to the mermaid library version, configuration, or load mode (eager vs lazy). The lazy-load investigation belongs to the deferred MSW leak hunt, not US024.
...
- **Soft dependency on US020** (FE hooks AbortController harmonisation). If US020 lands first, the `useDocument` refactor in US024 MUST NOT regress its AbortController contract — signal threading through `lib/api/`, cleanup on unmount, and abort-on-prop-change semantics must be preserved across the `useReducer` rewrite. If US024 lands first, US020 picks up the already-reduced `useDocument` and just confirms the signal threading is still in place. The system architect locks the ordering.

## Notes for the team

- Today's recorded baseline (tech-lead full scan, 2026-06-01): **92/100, 4 errors, 19 warnings.** ~70% of the state/effect findings live in REQ004-shipped files. US024 deliberately targets only the 4 errors (3 state/effect clusters + 1 security) plus their immediate `exhaustive-deps` knock-ons.
