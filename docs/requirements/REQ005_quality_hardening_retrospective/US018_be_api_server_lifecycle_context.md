# US018 — Signal-cancellable lifecycle context for `api-server` DB ping

**Story:** US018 — Replace unbounded `context.Background()` at DB-ping sites with signal-cancellable lifecycle context
**Requirement:** REQ005
**Track:** BE
...
- **Out:** `mcp-server` changes (separate task `US018_be_mcp_server_lifecycle_context.md`); shared `internal/lifecycle/` helper package (rejected by D-008); graceful HTTP shutdown / `e.Shutdown(ctx)` (D-009 — out of REQ005); adding `DB_PING_TIMEOUT_SECONDS` env var (D-013 — TODO comment only); sweeping `context.Background()` in test files (D-003).
...
This task and `US018_be_mcp_server_lifecycle_context.md` touch DIFFERENT files (different `cmd/<binary>/` paths). Per architecture §2 US018 and D-008, NO shared helper is extracted, so there is no overlap on `internal/` files. Both US018 tasks can run in parallel.

## Test contract

The dev must make these tests pass (from `US018_be_unit_tests.md`, IDs assigned by tester):
...
- Tests passing: UT-US018-API-001 (`TestPingDB_TimeoutCancels`), UT-US018-API-002 (`TestPingDB_CancellationPropagates`), UT-US018-API-003 (`TestPingDB_HappyPath`), UT-US018-API-004 (`TestMain_NoContextBackgroundAtPingSite`). Total 111 tests in `services/agent-board` (107 pre-existing + 4 new). `go vet` clean, `gofmt -s` clean.
- New file: `services/agent-board/cmd/api-server/main_test.go`.
- Worktree branch: `worktree-agent-ab14e3daf8d822fa2`. Head: `d4be789`.
- Implementation note: `go-sqlmock v1.5.2` does not preserve exact context error when `WillDelayFor` races with context expiry, so timeout/cancellation tests use pre-expired/pre-cancelled contexts (database/sql's context check fires before the mock driver). UT-US018-API-004 is a structural source assertion.

### Review pass 1 — 2026-06-02 — tech-lead (inline orchestrator review) — verdict: approved

- **NOTE on review modality:** Tech-lead subagent reviews aborted with `STALE_WORKTREE_BASE` — harness forks worktrees from origin `e233b20`, not local `main` (`d3d33e4`). This is the very bug US023 was scoped to fix. Inline orchestrator review used as recovery; gates run against local working tree which has dev work committed.
- `cd services/agent-board && go test ./cmd/api-server/...`: **4/4 PASS**. All UT-US018-API-001..004 green.
...
- No new tech_debt entries (clean implementation; sibling US018_be_mcp_server still pending).
