...
**Story:** US017 — Soft-warn `gosec` / `govulncheck` when missing instead of hard-exit
**Requirement:** REQ005
**Track:** BE
...
- **In:** Edits to `scripts/review/run-gate.sh` `gate_be()` — replace the `require_tool gosec ...` and `require_tool govulncheck ...` calls with inline `command -v` checks per architecture §2 US017 row and D-011. Add a "Soft-warn vs. hard-required" subsection to `scripts/review/README.md` listing hard tools (go, golangci-lint, npm, semgrep, gitleaks) vs soft-warn tools (gosec, govulncheck) plus install one-liners.
- **Out:** Installing `gosec` or `govulncheck` in CI or a Makefile target; changing `.golangci.yml`; the `printf` fix (US015); the `--forceExit` fix (US016); refactoring `require_tool` for cross or FE tracks.
...
Same `run-gate.sh` as US015 and US016 — see note in US016 about queue-time serialisation. README.md is unique to this task.

## Test contract

The dev must make these tests pass (from `US017_be_unit_tests.md`, IDs assigned by tester):
...
- Pseudocode shape:
  ...
  ```
  (Note the `printf --` form — US015's fix.)
...
- Code matches architecture §2 US017 row and §9 D-011.
- Dev set status to `in_review` and reported back; tech-lead approved (status flipped to `completed`).

## Notes

**Files touched:**
- `scripts/review/run-gate.sh` — replaced `require_tool gosec` and `require_tool govulncheck` in `gate_be()` with inline `command -v` checks; prints `WARN` + install one-liner and skips the `run_check` when absent. `go` and `golangci-lint` `require_tool` calls preserved as hard-required. Removed stale comment on pushd line.
- `scripts/review/README.md` — added "Soft-warn vs. hard-required" subsection under "What it runs" listing hard tools (go, golangci-lint, npm, semgrep, gitleaks) vs soft-warn tools (gosec, govulncheck), explaining gosec coverage via golangci-lint's built-in linter, and providing install one-liners for both soft-warn tools.
- `scripts/review/test/test_run_gate.sh` — extended with IT-US017-001 through IT-US017-006 (and IT-US017-004a/b split).

**Tests added:** IT-US017-001, IT-US017-002, IT-US017-003, IT-US017-004a, IT-US017-004b, IT-US017-005, IT-US017-006 (7 new assertions). All 14 tests in harness pass.

**Test harness output:** `bash scripts/review/test/test_run_gate.sh` → `14 passed, 0 failed — ALL TESTS PASSED`.

**Architecture compliance:** inline `command -v` pattern per §2 US017 row and §9 D-011. No new helper function added (two call sites only). `printf --` form used consistently per US015 fix.
...
- `bash scripts/review/test/test_run_gate.sh`: **14/14 PASS** (7 from US015/US016 + 7 new IT-US017-001..006 incl. -004a/-004b split).
- Architecture §2 US017 + D-002 honored: soft-warn for `gosec`/`govulncheck`, hard-required preserved for `go`/`golangci-lint`/`semgrep`/`gitleaks`/`npm`.
- IT-US017-005 confirms soft-warn logic is confined to `gate_be()` — cross/FE gates unaffected.
- IT-US017-006 confirms README "Soft-warn vs. hard-required" subsection landed.
