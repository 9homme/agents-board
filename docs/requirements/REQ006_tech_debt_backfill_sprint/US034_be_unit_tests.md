# US034 — Backend unit & integration test specification
...
- **Then:**
  - `assert.Equal(t, "", url)`
  - `assert.EqualError(t, err, "DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US034)")`
- **Exact error string (locked in architecture.md §5.4):** `"DB_URL is no longer supported; rename to DATABASE_URL (REQ006/US034)"`
...
- **Then:**
  - `assert.Equal(t, "", url)`
  - `assert.EqualError(t, err, "DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US034)")`
- **Exact error string (locked in architecture.md §5.4):** `"DB_URL is set but no longer supported; remove DB_URL from your environment to disambiguate (DATABASE_URL is the sole accepted name as of REQ006/US034)"`
