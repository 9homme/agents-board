...
|---|---|---|---|---|
| Gate invocation string contains `--forceExit` after `--watchAll=false` | integration | IT-US002-001 | `scripts/review/run-gate.sh` | FE Jest invocation includes `--forceExit` |
| Failing FE test reports FAIL and does not hang | integration | IT-US002-002 | `scripts/review/run-gate.sh` | gate exits non-zero with Jest failure; no timeout hang |
| No other Jest-invoking script is silently modified | integration | IT-US002-003 | `scripts/review/run-gate.sh` | scope is limited to the FE-gate line |

## Integration tests

### IT-US002-001 — Gate invocation contains `--forceExit`
...
- **Architecture cite:** architecture §2 US002 row — edit line 116 of `run-gate.sh` to append `--forceExit`.

### IT-US002-002 — Gate terminates within timeout on FE Jest completion
...
- **Notes:** This test is the definitive proof that the flag fixes the MSW/mermaid handle-leak hang. If the test environment does not have Node.js, mark as a manual verification step in the test report; that is acceptable provided IT-US002-001 is automated.
- **Architecture cite:** US002 AC "Scenario: FE gate terminates cleanly with `--forceExit`".

### IT-US002-003 — Only the FE gate line is modified; no other Jest invocations changed
...
- **Architecture cite:** US002 AC "Scenario: no other Jest invocation regressed".
