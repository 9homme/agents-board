...
- **Then:**
  - `result` is `nil`
  - `err` is non-nil
  - `err.Error()` contains `"failed to query audit trail"` (architecture.md §US003 AC wrap text, line 34 of `audit_repo.go`)
...
- **Then:**
  - `result` is `nil`
  - `err` is non-nil
  - `err.Error()` contains `"failed to scan audit trail entry"` (architecture.md §US003 AC wrap text, line 42 of `audit_repo.go`)
  - `mock.ExpectationsWereMet()` returns nil
...
- **Then:**
  - `result` is `nil`
  - `err` is non-nil
  - `err.Error()` contains `"error iterating audit trail"` (architecture.md §US003 AC wrap text, line 47 of `audit_repo.go`)
  - `mock.ExpectationsWereMet()` returns nil
