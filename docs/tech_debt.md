...
| 14 | 2026-06-10 | `US044_be_requirement_schema_migration_domain.md` `## Architecture extract` (path block, lines 110-111) | "Verbatim" SQL `ADD COLUMN path TEXT NOT NULL DEFAULT ''` + `UNIQUE(path)` throws 23505 unique-violation when 2+ projects pre-exist (all get `''`). Dev correctly deviated to nullable→backfill `id::text`→NOT NULL→UNIQUE. | Fix the extract's path SQL to the safe 3-step pattern so future readers don't copy the broken verbatim block | REQ008/US044 |
| 15 | 2026-06-10 | `US044_be_unit_tests.md` UT-044-004 | Spec lists `TaskCount` as a pre-existing `UserStory` field; it never existed on the struct — minor spec inaccuracy (no behavioral/coverage impact) | Drop `TaskCount` from the field list on next touch | REQ008/US044 |
| 16 | 2026-06-10 | `services/agent-board/internal/migrate/migration_it_test.go` | IT-044-* skip (not fail) when no local Postgres reachable; CI without a DB would green-wash these integration tests | Ensure REQ Quality Gate / CI provisions Postgres so IT-044-* execute, not skip | REQ008/US044 |
