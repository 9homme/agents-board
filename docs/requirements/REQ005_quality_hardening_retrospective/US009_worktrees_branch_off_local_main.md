# US009 — Sub-agent worktrees branch off local `main` (harness preferred; agent-definition fallback)
...
- **Scenario: README captures the chosen path and the reason**
  ...
  - Then a `### Decision: US009 path` entry is appended to the README (or to a `docs/operations/harness.md` file) naming which path (a or b) shipped, and why the other was infeasible if applicable
