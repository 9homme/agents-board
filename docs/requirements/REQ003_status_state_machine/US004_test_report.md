...
**Story:** US004 — Tech debt: drive `golangci-lint` to zero in `services/agent-board/`
...
## BE — UT-* spec coverage (from `US004_be_unit_tests.md`)
...
## FE — FCT-* coverage (from `US004_fe_unit_tests.md`)
...
## E2E — E2E-* coverage (from `US004_e2e_tests.md`)

**N/A — no behavioural change in production code.** Existing US001 / US002 / US003 e2e suites remain the regression guard for the state-machine behaviour. No new `.robot` files were created for US004. No E2E-* test cases apply.
...
|---|---|---|---|
| `US004_be_unused_handler_test_triage.md` | `unused` (9 → 0, already-zero on re-baseline) | completed (pass 1, no-op) | `5dc20c1` |
| `US004_be_mechanical_noctx_errorlint.md` | `noctx` (11) + `errorlint` (3) | completed (pass 1) | `5a0857a` |
| `US004_be_errcheck_rollback_discard.md` | `errcheck` (4) | completed (pass 1) | `5b55e6e` |
| `US004_be_tail_gocritic_gosec_revive.md` | `gocritic` (5) + `gosec` (1) + `revive` (1); owns story-wide exit-0 gate | completed (pass 1) | `912724e` |
