# US011 — Tech debt: drive golangci-lint to zero in services/agent-board
...
## Dependencies
- REQ003 US008, US009, US010 (all `done`) — this story cleans up debt in the same service those stories built.
...
**Spec review (UT-001..UT-010 coverage in `US011_be_unit_tests.md`):**
...
- `US011_fe_unit_tests.md` and `US011_e2e_tests.md` are appropriate N/A stubs — this is a backend quality refinement with no UI surface and no API contract change. The N/A decision is honoured correctly.
...
**Tester coverage acknowledgement:** The 10 UT-* static-analysis cases in `US011_be_unit_tests.md` (including the per-linter `--enable-only` verification commands and the UT-010 grep-contract for suppression hygiene) were the right shape for a quality-refinement story with no new behaviour, and the test report demonstrates each one as PASS with a per-linter cross-check table. Good spec hygiene; no spec gap.

**Provenance — the four task review commits referenced in the test report (chain integrity verified):**
- `5dc20c1` — `US011_be_unused_handler_test_triage.md` review pass 1 approved (verified no-op due to PR #1 drift)
- `5a0857a` — `US011_be_mechanical_noctx_errorlint.md` review pass 1 approved (`noctx` 11 + `errorlint` 3 → 0)
- `5b55e6e` — `US011_be_errcheck_rollback_discard.md` review pass 1 approved (`errcheck` 4 → 0)
- `912724e` — `US011_be_tail_gocritic_gosec_revive.md` review pass 1 approved (`gocritic` 5 + `gosec` 1 + `revive` 1 → 0; owns the story-wide exit-zero gate)
