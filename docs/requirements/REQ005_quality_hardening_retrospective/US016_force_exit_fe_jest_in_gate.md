...
- **Scenario: failing FE test still reports correctly**
  ...
  - And the failure output is captured and printed (combined with US015's `printf` fix to ensure visibility)
...
## Out of scope
- **Fixing the actual MSW / mermaid handle leak.** This story is the surgical unblock; the real leak hunt is intentionally left for a follow-up story (see REQ005 README "Open questions" — may become US024).
...
## Dependencies
- Soft: US015 should land first so that if a Jest failure does occur, the captured output is actually printed. Not a hard blocker — the two stories can ship in either order.
...
- **Evidence the leak is real** is in the audit: per-task reviews for REQ004 US015/US016/US017 FE work documented the hang; Jest's `A worker process has failed to exit gracefully` warning has been observed even on the audit run that did not hang.
