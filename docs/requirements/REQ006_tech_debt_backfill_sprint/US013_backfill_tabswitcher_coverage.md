...
- **Scenario: closes tech-debt finding**
  ...
  - Then `docs/tech_debt.md` line 75 is struck through with `→ fixed in REQ006/US013`
...
- **Coverage threshold is 80% (not 95%).** REQ005/US005's BE-test 95% was set because `sqlmock` makes every `repo` branch reachable. FE component branches sometimes hit React internals that are harder to exercise without unrealistic setups. 80% is the project's existing FE-coverage convention for component tests; tester / fe-dev may push higher if achievable.
