...
3. **Run the mandatory review gate** — static analysis for quality + security. This is a non-negotiable layer on top of unit tests: linters, security scanners, dependency-vuln checks, and CSR-only enforcement.
   ...
   - **Live e2e run + flake verification (mandatory — added 2026-06-03 per REQ005/US008 follow-up).** Bring up the e2e stack (`make e2e-up && make e2e-seed`) and run `make e2e-run` THREE consecutive times. **All three runs MUST be 100% green — no failures, no flakes.** A single failure in any of the 3 runs that goes green in the others is a **FLAKE**, which is `changes_requested` (or `SPEC_GAP_FOUND` if the flake is in the test code rather than the application code). Paste all three `N tests, N passed, 0 failed` summary lines verbatim into the review-log entry. Do NOT pass-with-rationalisation ("the flake is unrelated to this task," "flaky in the full suite but green when isolated") — a flake means the test pyramid is lying and that is exactly the REQ005 pattern we are closing. If the e2e stack itself is unavailable on the review host, that's `blocked_review_gate` — NOT `approved`. The dev's `## Notes` must already contain ONE clean run from step 8a of their hand-off; your 3 runs are on top of that.
   ...
   - **You cannot issue `approved` without pasting the gate's final `REVIEW GATE: PASS` line, the coverage per-file numbers, (if applicable) the robot dryrun summary, AND the three consecutive live-e2e run summary lines (all `0 failed`)** verbatim into the `### Review pass N` entry. The orchestrator rejects a review pass that doesn't include all four pieces of evidence.
